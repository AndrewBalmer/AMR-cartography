...
# 1) Reference (2-0-2) centroid on both maps
...
centroids_by_mlst <- processed_data %>%
  ...
  dplyr::mutate(
    phen_centroid_x_202 = centroids_202$phen_centroid_x_202,
    phen_centroid_y_202 = centroids_202$phen_centroid_y_202,
    gen_centroid_x_202  = centroids_202$gen_centroid_x_202,
    gen_centroid_y_202  = centroids_202$gen_centroid_y_202,
    phen_dist_to_202    = sqrt((D1_med - phen_centroid_x_202)^2 + (D2_med - phen_centroid_y_202)^2),
    gen_dist_to_202     = sqrt((G1_med - gen_centroid_x_202)^2 + (G2_med - gen_centroid_y_202)^2)
  )
...
background_centroids <- centroids_by_mlst %>%
  dplyr::filter(!(MLST %in% top_12_MLSTs))

# 3b) BIG POINTS computed EXACTLY like your summary table
...
# Palette mapping for ordered top-12 labels
palette_vec <- setNames(my_colours[seq_along(order_levels12)], order_levels12)
# 4) Plot — x hard cut, y hard cut; thicker border; extra RIGHT margin for "120"
x_major <- 10
y_major <- 2
...
p_centroids_top12 <- ggplot() +
  geom_point(
    ...
    aes(x = gen_dist_to_202, y = phen_dist_to_202),
  ...
  theme(
    ...
    # >>> add some breathing room on the RIGHT so "120" isn't clipped <<<
