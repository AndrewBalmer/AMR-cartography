...
# Reshape using pivot_longer and add an id column to preserve row order
table_distances <- table_distances %>%
  mutate(id = row_number()) %>%
  pivot_longer(cols = -id, names_to = "antibiotic", values_to = "table_distance")

map_distances <- map_distances %>%
  mutate(id = row_number()) %>%
  pivot_longer(cols = -id, names_to = "antibiotic", values_to = "map_distance")

# Combine table and map distances by id and antibiotic
distances <- left_join(table_distances, map_distances, by = c("id", "antibiotic")) %>%
  mutate(table_distance = as.numeric(table_distance),
         map_distance   = as.numeric(map_distance))

# Fit linear model: map_distance ~ table_distance
mapvtable <- lm(map_distance ~ table_distance, data = distances)
summary(mapvtable)
slope <- as.numeric(coef(mapvtable)[2])
dilation <- 1 / slope  # dilation factor so that 1 unit on the map ~ 1 MIC unit

# Set column names of the configuration and apply dilation
...
theta <- AMRC_MAP_ROTATION_DEGREES * pi / 180  # convert degrees to radians
rot <- matrix(c(cos(theta), sin(theta), -sin(theta), cos(theta)), ncol = 2)
torg_met$conf <- torg_met$conf %*% rot  # rotated configuration

# Modify distances tibble: scale map_distance and calculate residuals
distances <- distances %>%
  mutate(map_distance = map_distance * dilation,
         residuals    = table_distance - map_distance)

# Compute goodness-of-fit metrics
perc_above_1 <- sum(abs(distances$residuals) > 1) / nrow(distances) * 100
perc_above_2 <- sum(abs(distances$residuals) > 2) / nrow(distances) * 100
res_mean     <- mean(abs(distances$residuals), na.rm = TRUE)
res_sd       <- sd(abs(distances$residuals), na.rm = TRUE)
