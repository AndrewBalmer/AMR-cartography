...
# ==== Pairwise plot: keep all tick marks, label only min & max (µg/mL) ====
...
# Define breakpoints (convert to log2 scale)
breakpoints <- c(
  Penicillin = 1,
  Cefuroxime = 1,
  Ceftriaxone = 1,
...
# Helpers:
# - full_breaks: all integer log2 steps across panel range (for tick marks)
# - minmax_labeler: show labels only for min & max ticks (values in µg/mL)
...
# Custom lower panel: scatter with breakpoints, rotated x-axis
tick_size <- 18

my_lower <- function(data, mapping, ...) {
  xvar <- as_label(mapping$x); yvar <- as_label(mapping$y)
  x_break <- log_breakpoints[xvar]; y_break <- log_breakpoints[yvar]
  xb_full <- full_breaks(data[[xvar]])
  yb_full <- full_breaks(data[[yvar]])

  ggplot(data, mapping) +
    geom_vline(xintercept = x_break, linetype = "solid", color = "#E41A1C") +
    geom_hline(yintercept = y_break, linetype = "solid", color = "#E41A1C") +
...
# Custom diagonal panel: histogram with vertical breakpoint line
my_diag <- function(data, mapping, ...) {
  xvar <- as_label(mapping$x); x_break <- log_breakpoints[xvar]
  xb_full <- full_breaks(data[[xvar]])
...
# Custom upper panel: correlation (unchanged)
...
# Print and save
print(p)
ggsave("pairwise_plot.png", plot = p, width = 10, height = 10, dpi = 300)
