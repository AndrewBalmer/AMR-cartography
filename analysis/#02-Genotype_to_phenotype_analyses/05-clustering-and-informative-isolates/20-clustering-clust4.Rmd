...
For the analysis in this document, I focus attention on cluster 9. I have primarily only worked with this group so far, but I plan to conduct the same analysis on the other clusters. My preliminary results on other groups suggest it is possible to complete similar analyses with the other genetic clusters.

Across the whole dataset there are 914 PBP positions, of which 286 are variant. Within cluster 9, there are 94 PBP loci which vary, any of which could potentially be associated with phenotypic change. However, many of these substitutions will simply be hitchhikers associated with particular phenotypes by chance, or potentially be fitness compensatory mechanisms. The challenge is in identifying the causal substitutions among the 94 varying positions.
...
Below is an example of this method. The first plot shows the AA positions of the three PBPs on the X axis. The Y axis shows the absolute proportion change in frequency of an amino acid residue at that position. In this simple case, there is one location: PBP2X_I371, where 100% of the isolates in cluster 2 have isoleucine at this position, whereas 100% of isolates of cluster 1 have Threonine at this position. The phenotypic difference between the two clusters can then be attributed to the variation at this position. Notably, this position was also picked out in the 'Identifying single substitutions' analysis, where it generated a phenotypic distance of 4.43. Isolates of cluster 9 which belong to the other phenotype clusters are marked as large grey points.
...
In this second case, there are again no locations which strongly differentiate clusters 1 and 3. Position 2X 468 shows a moderate degree of differentiation between the clusters, but again the clusters are too close in phenotypic space to be able to precisely pick out differences.

I have made a note of this position, but the phenotypic difference between the two clusters cannot necessarily be attributed to this position.
...
There are two positions 2X 561 and 551 which strongly differentiate clusters 1 and 4. Both positions came out as significant in the LMMs and Position 551 in Nazreens work. Given the larger phenotypic distance between the two clusters, it seems possible to pick out the substitutions in this case. I have made a note of these position as having strong evidence for an effect.
...
cluster_1_cluster_4_difference <- cluster_diff_subs %>% select(Location, AA_max, cluster_1_cluster_4_max) %>%
  filter(cluster_1_cluster_4_max >= .8)
...
Here, there are again no locations which strongly differentiate clusters 2 and 3. Position 2X 468 shows a moderate degree of differentiation between the clusters, but again the clusters are too close in phenotypic space to be able to precisely pick out differences. Many of the isolates of both clusters have threshold values to penicillin, making it more difficult to pick out true phenotypic differences between the isolates.

Below, I have coloured the points by the AA at poosition 2B 414.
...
## Example 5 --- comparing phenotype clusters 2 and 3

As with the comparison between clusters 1 and 4, the two positions 2X 561 and 551 come out as having a clear difference between clusters 2 and 4.  I have made a note of these position as having strong evidence for an effect.
...
In this last comparison, there was a clear difference in amino acid at position 2X 568 between clusters 2 and 4. Position 2X 551 also again differentiated strongly, but not 100%, due to a single PBP type in cluster 3 having amino acid S at this position.
...
# Summary
In total, 6 PBP locations were strongly associated with the phenotypic cluster differences in this subset of isolates. All 6 were located in the PBP2x protein. 

## Visualising the identified substitutions
Below I have plotted the 6 cluster-difference locations from within genetic cluster 9. Each of these locations appears strongly associated with particular areas of the map. One possible exception is PBP2X_T230, which was the cluster difference location between clusters 4 and 1, which was less clear from the analysis above. 
