...
### cluster 1
For the analysis in this document, I focus attention on cluster 1. Across the whole dataset there are 914 PBP positions, of which 286 are variant, Any of which could potentially be associated with phenotypic change. However, many of these substitutions will simply be hitchhikers associated with particular phenotypes by chance, or potentially be fitness compensatory mechanisms. The challenge is in identifying the causal substitutions among the 286 varying positions.
...
ggplot(filter(AA_matrix_phenotype, gen_cluster == 1 ), aes(x=G1,y=G2, fill = gen_cluster)) +
...
cluster_5 <- filter(AA_matrix_phenotype, gen_cluster == 1)



## Pull out the AA sequences for the isolates
PBP_positions <- cluster_5 %>% 
  ungroup() %>%
  select(starts_with("PBP"), phen_cluster) %>%
  select(-PBP_type)

    
limits_phen_map_x_cluster <- c(min(cluster_5$D1, na.rm = T), 
                       max(cluster_5$D1, na.rm = T))
limits_phen_map_y_cluster <- c(min(cluster_5$D2, na.rm = T), 
                       max(cluster_5$D2, na.rm = T))

breaks_phen_map_x_cluster <- seq(min(cluster_5$D1, na.rm = T) - (1), 
                         max(cluster_5$D1, na.rm = T) + (1), 1)
breaks_phen_map_y_cluster <- seq(min(cluster_5$D2, na.rm = T) - (1), 
                         max(cluster_5$D2, na.rm = T) + (1), 1)


limits_gen_map_x_cluster <- c(min(cluster_5$G1, na.rm = T) - (gen_slope * 5), 
                      max(cluster_5$G1, na.rm = T) + (gen_slope * 5))
limits_gen_map_y_cluster <- c(min(cluster_5$G2, na.rm = T) - (gen_slope * 5), 
                      max(cluster_5$G2, na.rm = T)+ (gen_slope * 5))

breaks_gen_map_x_cluster <- seq(min(cluster_5$G1, na.rm = T)- (gen_slope * 10), 
                        max(cluster_5$G1, na.rm = T) + (gen_slope * 10), gen_slope * 10)
breaks_gen_map_y_cluster <- seq(min(cluster_5$G2, na.rm = T)- (gen_slope * 10), 
                        max(cluster_5$G2, na.rm = T) + (gen_slope * 10), gen_slope * 10)

ggplot(filter(cluster_5, gen_cluster == 1 ), aes(x=D1,y=D2, fill = gen_cluster)) +
    geom_point(data = select(all_isolates, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
...
nb <- NbClust(data = select(PBPclustering, x_centroid, y_centroid), distance = "euclidean",
        min.nc = 2, max.nc = 5, method = "ward.D")
```
...
ggplot(filter(cluster_5, gen_cluster == 1 ), aes(x=x_centroid,y=y_centroid, fill = PBP_type)) +
  geom_point(data = select(all_isolates, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
...
ggplot(filter(cluster_5, gen_cluster == 1 ), aes(x=G1,y=G2, fill = PBP_type)) +
    geom_point(data = select(all_isolates, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
  geom_point(shape = 21,size = 5, alpha = .6) +
  theme_linedraw() +
...
ggplot(filter(AA_matrix_phenotype, gen_cluster == 1 ), aes(x=D1,y=D2, fill = PBP_type)) +
  geom_point(data = select(AA_matrix_phenotype, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
  geom_point(shape = 21,size = 4, alpha = 1) +
  geom_point(cluster_5, mapping = aes(x_centroid, y_centroid, colour = "#000000"),
...
I then used the hierarchical clustering algorithm on the centroid positions to pick out the 3 regions which have different phenotypes within this group of low PBP genetic diversity. Here, the centroids and the isolates which belong to that centroid are coloured by their phenotype cluster.

```{r, echo =F}

ggplot(filter(cluster_5), aes(x=D1,y=D2, fill = phen_cluster)) +
  geom_point(data = select(all_isolates, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
    geom_point(shape = 21,size = 4, alpha = 1) +
  geom_point(cluster_5, mapping = aes(x_centroid, y_centroid, colour = phen_cluster),
...
## Example 2 - comparing clusters 2 and 4
Here is a second, slightly more complex example, where there are four locations which vary by 100% between the clusters 2 and 4. The two combinations are marked out below on the map. In this case, it may be one or all of the four substitutions which induce the phenotypic change between the two clusters. It may be possble to infer which of the four are more likely to be causal substitutions when looking at other areas of the genotype map. Alternatively, experimental work or searching for isolates with different AA combinations at these four positions, may make it possible to pick out which of the four are causal.
...
cluster_1_cluster_2_difference <- cluster_diff_subs %>% select(Location, AA_max, cluster_1_cluster_2_max) %>%
  filter(cluster_1_cluster_2_max >= .1)
...
cluster_5_diff <- left_join(cluster_5, select(cluster_1_cluster_2_difference, LABID, Profile_clusters_1_2), by = c("LABID")) %>% ungroup()



### Identifying informative isolates --- cluster 2 to 1
A <- ggplot(filter(cluster_diff_subs, cluster_1_cluster_2_max >= 0.1), aes(x = reorder(Location, cluster_1_cluster_2_max, FUN =max), y = cluster_1_cluster_2_max, group = 1)) +
...
C <- ggplot(filter(cluster_5_diff, phen_cluster == 2 | phen_cluster == 1), aes(x=G1,y=G2, fill = Profile_clusters_1_2)) +
    geom_point(data = select(cluster_5_diff, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
...
#Cluster_substitutions <- as.data.frame(c(Cluster_substitutions, colnames(cluster_1_cluster_4_difference[,11:ncol(cluster_1_cluster_4_difference)])))
#colnames(Cluster_substitutions) <- "Location"

#Cluster_substitutions$Genetic_cluster <- "1"
#Cluster_substitutions$Phenotypic_cluster <- "1_4"
#Cluster_substitutions$Confidence <- "Strong"
...
However, cluster 1 represents a low number of PBP-types, which are not phenotypically distinct. In this case, the method will struggle to pick out substitutions which are strongly associated with phenotypic change. In this cluster, I identified several substitutions which were associated with the phenotypic change between clusters, however these were marked as having only weak evidence, because the pbp-types were mostly overlapping in their phenotypic distributions. 
...
#write.csv(Potential_substitutions, file = "Sig_AA_subs_Cluster_gen1_S.pneumo.csv", row.names=FALSE)
...
Potential_substitutions <- Potential_substitutions %>%
  pivot_longer(!D1:y_centroid, names_to = "Location", values_to = "AA")  %>% 
  separate(Location, c("PBP","Loci"), remove = F)


ggplot(filter(Potential_substitutions, gen_cluster == 1 ), 
       aes(x=D1,y=D2, fill = AA)) +
    geom_point(data = AA_matrix_phenotype, 
             colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1) +
