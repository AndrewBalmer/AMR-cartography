...
I then used the hierarchical clustering algorithm on the centroid positions to pick out the 2 regions which have differing phenotypes within this group of low PBP genetic diversity. Here, the centroids and the isolates which belong to that centroid are coloured by their phenotype cluster.
...
In this example, there are no substitutions which clearly differentiate the two clusters, most likely because there is substantial overlap in distribution between the two groupings. 
...
# Summary
In total, no PBP locations were strongly associated with the phenotypic cluster differences in this subset of isolates. This is likely because the two clusters are phenotypically very similar to one another.
