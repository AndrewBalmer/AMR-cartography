...
core_ranges <- rep(ceiling(no_noise_samples / num_cores), num_cores)

# Define a function to perform MDS on a chunk of datasets
run_mds <- function(indices) {
  noise_values_test <- list()
  
  for (i in 1:no_noise_samples) {
    temp_MDS <- mds(noise_added_samples_dists[[i]], ndim = 2, type = c("ratio"), 
                    init = "torgerson", 
                    modulus = 1, itmax = 1000, eps = 1e-06)
    
    noise_values_test[[i]] <- list(conf = temp_MDS$conf, stress = temp_MDS$stress, spp = temp_MDS$spp)
    temp_MDS <- NULL
  }
  
  return(noise_values_test)
}

# Run MDS in parallel
noise_samples_mds_objects <- foreach(indices = core_ranges, .combine = "c") %dopar% {
  run_mds(1:no_noise_samples)
}
...
############################################################################################################
### double check this is working correctly - as in it is not summing error across datasets
############################################################################################################
...
# Create a new column 'abs_error_added' based on a condition
stress_per_point_comparison_2 <- stress_per_point_comparison %>% 
  mutate(abs_error_added = ifelse(abs_error_added == "1", "Additional error", "No additional error"))

# Calculate the mean of 'dist_phen'
mean(stress_per_point_comparison_2$dist_phen)

# Group by 'abs_error_added' and calculate mean and median of 'dist_phen'
mu <- stress_per_point_comparison_2 %>%
  dplyr::group_by(abs_error_added) %>%
  dplyr::summarize(Mean = mean(dist_phen, na.rm = TRUE),
            Median = median(dist_phen, na.rm = TRUE))

# printed the rounded mean value
round(mu$Mean[1], 3)

# Create a ggplot for visualizing the distribution of 'dist_phen'
C <- ggplot(stress_per_point_comparison_2, aes(dist_phen, y = stat(density), colour = abs_error_added, fill = abs_error_added)) + 
  geom_histogram(position="identity", alpha = 0.45, color = "black") + 
...
ggplot(filter(stress_per_point_comparison, n != 0 &  dataset == 1 ), aes(x=D1_X, y=D2_X)) + 
  geom_point(data = filter(stress_per_point_comparison, n == 0 & dataset == 1), aes(x=D1_X, y=D2_X), shape = 4, fill = "grey", size = 1.5, alpha = 0.6)+
    geom_segment(data = filter(stress_per_point_comparison, n != 0 & dataset == 1), aes(x = D1_X, y = D2_X, xend = D1_Y, yend = D2_Y), size = .5, colour = "grey") + 
  geom_point(data = filter(stress_per_point_comparison, n != 0 & dataset == 1), shape= 21, fill = "black", size = 2, colour = "white") + 
  geom_point(data = filter(stress_per_point_comparison,n != 0 & dataset == 1), aes(x=D1_Y, y=D2_Y), shape = 21, fill = "#E41A1C", size = 2, colour = "white") +
  theme_bw() +   
  #facet_wrap(~ dataset)+
...
for (f in 1:4){
noise_added_samples_dists_dimensions[[f]] <- list()
}
...
temp_MDS <- mds(noise_added_samples_dists[[i]], ndim = f, type = c("ratio"), init = "torgerson", modulus = 1, itmax = 1000, eps = 1e-06)

noise_added_samples_dists_dimensions[[f]][[i]] <- list()

noise_added_samples_dists_dimensions[[f]][[i]]$conf <- temp_MDS$conf
noise_added_samples_dists_dimensions[[f]][[i]]$stress <- temp_MDS$stress
noise_added_samples_dists_dimensions[[f]][[i]]$spp <- temp_MDS$spp
temp_MDS <- NULL
...
#noise_samples_dists_dimensions <- readRDS(file = "noise_samples_dists_dimensionality_test.rds")
...
torg_met_dimensions <- readRDS(file = "/Users/ajb306/AMR-cartography/analysis/#01-Phenotype_and_map_analyses/#03-06-Generating-phenotype-and-genotype-maps/torg_met_dimensions_missing_values.rds")
