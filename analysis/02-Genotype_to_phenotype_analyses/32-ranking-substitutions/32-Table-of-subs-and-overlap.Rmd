...
> published-manuscript script and its logic is retained **unchanged**. It builds
> the ranked substitution / evidence table (Supplementary File 1) from the
> additive mvLMM, epistatic mvLMM, single-substitution, and clustering streams.
> The mvLMM and epistasis inputs were subsequently **re-run on a corrected
> 170-marker PBP panel** (the original 157-marker panel omitted 13 valid markers)
> in a separate farm workflow at [`../recomputed_170_workflow/`](../recomputed_170_workflow/) (see
> `recomputed_170_workflow/README.md`); the single-substitution and clustering streams were
> carried over unchanged. In the corrected rebuild, Supplementary File 1 grows
> from 354 to 394 component-expanded rows and multi-method support from 88 to 117
> substitutions, while the sole Very Strong (I371T) and the 5 Strong
> substitutions are stable. Manuscript-facing tables should be taken from the
> corrected rerun outputs, not from this file. Only this note was added.
...
mv_lmm <- read_csv(amrc_pneumo_path("Sub_effect_sizes_mv_pneumo.csv"))
mv_lmm_epi_all <- read_csv(amrc_pneumo_path("Sig_AA_subs_all_epistatic_interactions_incl_nonsig_S.pneumo.csv"))
mv_lmm_epi <- read_csv(amrc_pneumo_path("Sig_AA_subs_all_epistatic_interactions_S.pneumo.csv"))
...
comparison_all <- read.csv(amrc_pyproject_path("comparison_all_output.csv")) %>%
