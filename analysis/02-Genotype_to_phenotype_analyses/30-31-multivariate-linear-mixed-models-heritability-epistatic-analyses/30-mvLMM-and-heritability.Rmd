...
> **Provenance / rerun note (added 2026-07-01).** This is the original,
> published-manuscript script and its analysis logic is retained **unchanged**.
> The additive mvLMM, univariate LMM, and epistatic mvLMM analyses implemented
> here were subsequently **re-run on a corrected 170-marker PBP panel** (the
> original 157-marker panel omitted 13 valid markers) in a separate,
> self-contained farm workflow at [`../recomputed_170_workflow/`](../recomputed_170_workflow/) (see
> `recomputed_170_workflow/README.md`). The rerun re-derives the Galwey effective-test
> counts, permutation thresholds, and epistasis interaction universe on the
> corrected panel. An independent audit found the recomputed additive p-values
> reproduce this script's values on the 157 shared markers to r ≈ 0.99999.
> Manuscript-facing tables should be taken from the corrected rerun outputs, not
> from this file. Only this note was added to the original.
...
# Specify the full path to the data file
gen_map <- amrc_data_path("Spneumo_3628_PCA_start_2D_METRIC_genetic.RData")
...
tablemic_metadata_MLST <- read.csv(amrc_data_path("MIC_S.Pneumo_metadata.csv"), header=TRUE, sep=",", skip = 0)
tablemic_metadata_MLST_post2015 <- read.csv(amrc_data_path("Meta_data_spneumoniae_isolates_post_2015.csv"), header=TRUE, sep=",", skip = 0) %>%
  rename(LABID = Sample,
         MLST = ST,
...
write.csv(All_pos, file = amrc_pneumo_path("Sub_effect_sizes_mv_pneumo.csv"), row.names=FALSE)
...
In this case, there are 914 positions across the three PBP TPD regions, of which 284 are variant. In my last markdown I coded the 284 positions as dummy variables and removed invariant, identical, and precisely opposite positions. I also removed the 'sensitive' representative AAs.
...
write.csv(uvLMM_V1_epi, file = amrc_pneumo_path("Sig_AA_subs_all_epistatic_interactions_incl_nonsig_S.pneumo.csv"), row.names=FALSE)

write.csv(Sub_effect_sizes_interaction, file = amrc_pneumo_path("Sig_AA_subs_all_epistatic_interactions_S.pneumo.csv"), row.names=FALSE)
...
> **Provenance / rerun note (added 2026-07-22).** The two `print()` values
> below (0.746, 0.597) were computed on the *original* 157-marker relatedness
> matrix and are **not** the manuscript-facing figures — they predate the
> 157→170 marker-panel correction described in the note at the top of this
> file. Re-running this same `limix.her.estimate` calculation on the
> corrected 170-marker relatedness matrix
> (`AMRC-repo-files/pythonProject1-additive-production-20260507-150112/S.pneumo_map_dummy_gen_relatedness_matrix.csv`)
> gives h² = 0.714 (axis 1) and 0.578 (axis 2), matching the manuscript and
> Supplementary Table S10's reported 71.4%/57.8%. Do not quote 0.746/0.597 as
> the current analysis's narrow-sense heritability.

```{r, echo = F}
print("Narrow sense heritability - Map axis 1: 0.746 (OLD 157-marker run; NOT the manuscript value, see provenance note above)")
print("Narrow sense heritability - Map axis 2: 0.597 (OLD 157-marker run; NOT the manuscript value, see provenance note above)")
