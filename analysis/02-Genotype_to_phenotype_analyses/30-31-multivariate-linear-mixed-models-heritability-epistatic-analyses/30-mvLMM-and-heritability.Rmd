...
#facet_wrap(~PBP, scales ="free_x") +
 ...
 #scale_y_continuous(limits = c( 0,8), breaks=seq( 0,8, .5)) +
  ...
  annotate("label", x = 3.25, y = -log10(0.001), label = "0.001")  +
  scale_colour_manual(values=c( "#E41A1C", "#377EB8"))


ggplot(filter(uvLMM_D1, pv20_adj_galwey <= 0.001), aes(x=effsize_env1_D1, y = effsize_env1_D2, size = -log10(pv20_adj_galwey))) + 
...
Across the three proteins, a total of 18 PBP locations were identified as has significant phenotypic effects, after correction for multiple comparisons. 
...
write.csv(All_pos, file = "Sub_effect_sizes_mv_pneumo.csv", row.names=FALSE)
...
After doing this I had 204 dummy variables. Coding the interaction terms resulted in a total of >8k interaction variables to be tested after removing invariant interactions. I then had a series of interaction variables with varying numbers of isolates representing each group. For example, an interaction with loci_A and loci_B would have four possible combinations  (a/b - 0_0, 0_1, 1_0, and 1_1). However, for many interactions, there was a very low number of isolates for each combination. I therefore wanted to exclude positions where I didnt have a good enough sample size to estimate effects. 
...
uvLMM_V1_epi$Significance <- ifelse(uvLMM_V1_epi$Significance <= 9.72e-06, "Significant", "Non-Significant")

uvLMM_V1_epi <- uvLMM_V1_epi %>%
  mutate(joint_effect_size = sqrt(effsize_env1_V1^2 + effsize_env1_V2^2),
         joint_effect_size_se = sqrt(effsize_se_env1_V1^2 + effsize_se_env1_V2^2))
...
ggplot(filter(uvLMM_V1_epi), aes(x=joint_effect_size, y = -log10(pv20_adj_galwey), size=lowest_number_of_isolates_by_comb)) + 
 ...
 geom_point(shape = 16, colour = "#E41A1C",  alpha = 0.25, position = position_jitter(width = 0., height = 0.)) +
  ...
  theme(axis.text =element_text(size=16), 
        axis.title=element_text(size=16)) +
  annotate("label", x = 4.5, y = -log10(0.001), label = "0.001")  + 
  scale_colour_manual(values=c( "#E41A1C", "#377EB8"))
...
ggplot(filter(uvLMM_V1_epi,pv20_adj_galwey< 9.72e-06 ), aes(x=effsize_env1_V1, y = effsize_env1_V2, size = joint_effect_size_se)) + 
  geom_point(shape = 16, colour = "#E41A1C",alpha = 0.25, 
             aes(), position = position_jitter(width = 0., height = 0.)) +
    geom_vline(xintercept=1) +
    geom_vline(xintercept=-1) +
...
No_sig_V1 <- uvLMM_V1_epi %>% 
  ...
  #filter(effsize_env1_V1 >= abs(1) | effsize_env1_V2 >= abs(1)) %>%
...
No_sig_V1_perm <- uvLMM_V1_epi %>% 
 filter(pv20_adj_galwey <= 9.72e-06) %>%
...
write.csv(uvLMM_V1_epi, file = "Sig_AA_subs_all_epistatic_interactions_incl_nonsig_S.pneumo.csv", row.names=FALSE)

write.csv(Sub_effect_sizes_interaction, file = "Sig_AA_subs_all_epistatic_interactions_S.pneumo.csv", row.names=FALSE)
...
I then plotted the number of significant interactions found at each position as a histogram. Most identifed positions have only one or two interactions. However, there is a subset of positions which have interactions with many other locations (mean in red) (see table below). I found one position had significant interactions with 21 other positions.
...
### tables of sig subs for each dimension
All_pos <- No_sig_V1 %>% 
  select(PBP_AA_location, pv20, pv20_adj_galwey, effsize_env1_V1, effsize_env1_V2, effsize_se_env1_V1,effsize_se_env1_V2, joint_effect_size) %>% distinct(PBP_AA_location, .keep_all = T) 
...
interaction_network <- All_pos %>% 
  select(c(PBP_AA_location, joint_effect_size, pv20_adj_galwey)) %>%
  separate(PBP_AA_location, c("AA1", "AA2"), ":", remove =F) %>%
...
geom_node_label(aes(label = PBP_AA_location$position), size = 3, hjust = 0.5, vjust = 0.5, margin = margin(.1, .1, .1, .1)) +
  ...
  #theme_bw()+
   ...
   scale_colour_manual(values = c("#FDBB84", "#EF6548" , "#B30000")) +
    ...
    #                                  face="bold")) +
    theme(legend.position='none') +
    annotate("label", x = .9, y = .9, label = "PBP1A") +
    annotate("label", x = 0, y = -1.25, label = "PBP2B")   +
    annotate("label", x = -.9, y = .9, label = "PBP2X")   
