...
# non-portable setwd() removed; knitr uses the document directory

# Read the MIC table data
tablemic <- read.csv(amrc_data_path("MIC_table_Spneumoniae.csv"), header=TRUE, sep=",", skip = 0)

# Read in the relevant meta data
tablemic_meta <- read.csv(amrc_data_path("meta_data_Spneumoniae.csv"), header=TRUE, sep=",", skip = 0)

# Specify the full path to the data file
phen_map <- amrc_data_path("Spneumo_3628_PCA_start_2D_METRIC.RData")

# Load the pre-computed PCA start 2D metric data
load(phen_map)

  res_samp <- torg_met
  theta <- AMRC_MAP_ROTATION_DEGREES * pi / 180 ## degrees to radians
...
# Specify the full path to the data file
load(amrc_data_path("Spneumo_3628_PCA_start_2D_METRIC_genetic.RData"))
load(amrc_data_path("tablemic_pneumo_gen_3628.RData"))
...
slope <- AMRC_PHEN_DILATION_SLOPE
dilation <- 1/slope
slope
...
gen_slope <- AMRC_GEN_DILATION_SLOPE
...
ggplot(filter(AA_matrix_phenotype, gen_cluster == 12), aes(x=G1,y=G2, fill = gen_cluster)) +
...
cluster_12 <- filter(AA_matrix_phenotype, gen_cluster == 12)



## Pull out the AA sequences for the isolates
PBP_positions <- cluster_12 %>% 
  ungroup() %>%
  select(starts_with("PBP"), phen_cluster) %>%
  select(-PBP_type)

    
limits_phen_map_x_cluster <- c(min(cluster_12$D1, na.rm = T), 
                       max(cluster_12$D1, na.rm = T))
limits_phen_map_y_cluster <- c(min(cluster_12$D2, na.rm = T), 
                       max(cluster_12$D2, na.rm = T))

breaks_phen_map_x_cluster <- seq(min(cluster_12$D1, na.rm = T) - (1), 
                         max(cluster_12$D1, na.rm = T) + (1), 1)
breaks_phen_map_y_cluster <- seq(min(cluster_12$D2, na.rm = T) - (1), 
                         max(cluster_12$D2, na.rm = T) + (1), 1)


limits_gen_map_x_cluster <- c(min(cluster_12$G1, na.rm = T) - (gen_slope * 5), 
                      max(cluster_12$G1, na.rm = T) + (gen_slope * 5))
limits_gen_map_y_cluster <- c(min(cluster_12$G2, na.rm = T) - (gen_slope * 5), 
                      max(cluster_12$G2, na.rm = T)+ (gen_slope * 5))

breaks_gen_map_x_cluster <- seq(min(cluster_12$G1, na.rm = T)- (gen_slope * 10), 
                        max(cluster_12$G1, na.rm = T) + (gen_slope * 10), gen_slope * 10)
breaks_gen_map_y_cluster <- seq(min(cluster_12$G2, na.rm = T)- (gen_slope * 10), 
                        max(cluster_12$G2, na.rm = T) + (gen_slope * 10), gen_slope * 10)

ggplot(filter(cluster_12, gen_cluster == 12), aes(x=G1,y=G2, fill = gen_cluster)) +
    geom_point(data = select(all_isolates, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
...
Here, I have plotted the isolates of cluster 12 on the phenotype map. Each colour represents a different genotype. The CDC uses a PBP typing scheme, where the genotype of each of the three PBPs is given a number. The labels represent the particular genotype combination for each of the three proteins.

```{r, echo = F}


ggplot(filter(cluster_12, gen_cluster == 12), aes(x=x_centroid,y=y_centroid, fill = PBP_type)) +
  geom_point(data = select(all_isolates, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
...
ggplot(filter(cluster_12, gen_cluster == 12), aes(x=G1,y=G2, fill = PBP_type)) +
    geom_point(data = select(all_isolates, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
  geom_point(shape = 21,size = 5, alpha = .6) +
  theme_linedraw() +
...
ggplot(filter(AA_matrix_phenotype, gen_cluster == 12), aes(x=D1,y=D2, fill = PBP_type)) +
  geom_point(data = select(AA_matrix_phenotype, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
  geom_point(shape = 21,size = 4, alpha = 1) +
  geom_point(cluster_12, mapping = aes(x_centroid, y_centroid, colour = "#000000"),
...
# Additional plotting for supplementary information

```{r, echo =F}

p1 <- ggplot(filter(cluster_12), aes(x=D1,y=D2, fill = phen_cluster)) +
  geom_point(data = select(all_isolates, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 1.25, alpha = 1)+
    geom_point(shape = 21,size = 2.25, alpha = 1) +
  geom_point(cluster_12, mapping = aes(x_centroid, y_centroid, colour = phen_cluster),
             shape = 4, size = 2.25, stroke = 1.25, show.legend =F) + guides() +
...
scale_x_continuous(limits = limits_phen_map_x, breaks=breaks_phen_map_x) +
 ...
 theme_MDR_cartography() +
    #theme(legend.position='none') 
  geom_label(inherit.aes=FALSE, size = 3, aes(x = 3.798163, y = 5.697245 ,label=paste("1"))) +
  geom_label(inherit.aes=FALSE, size = 3, aes(x = 8.387973, y = 2.033103,label=paste("2"))) +
  geom_label(inherit.aes=FALSE, size = 3, aes(x = 8.925684, y = 8.117218,label=paste("3"))) +
  geom_label(inherit.aes=FALSE, size = 3, aes(x = 11.88283, y = 3.5757,label=paste("4"))) +
  geom_label(inherit.aes=FALSE, size = 6, aes(x = -1.068912, y = 7.607179,label=paste("C"))) +
...
p2 <- ggplot(filter(cluster_12, gen_cluster == 12), aes(x=G1,y=G2, fill = phen_cluster)) +
...
coord_fixed() +    
  geom_label(inherit.aes=FALSE, size = 6, aes(x = 1.39, y = 0.522,label=paste("B"))) +
  scale_fill_manual(values=c("#4DAF4A", "#377EB8", "#E41A1C", "#984EA3", "#FF7F00", "black",
                             "white", "#7570B3", "#E7298A", "#FFFF33", "#A65628", "cyan",
                             "#999999", "violet","magenta", "purple", "brown","green","orange","magenta", "purple", "brown","green",
                             "#4DAF4A", "#377EB8", "#E41A1C", "#984EA3", "#FF7F00", "black",
...
p3 <-  ggplot(filter(AA_matrix_phenotype, gen_cluster == 12), aes(x=G1,y=G2, fill = phen_cluster)) +
    geom_point(data = select(all_isolates, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 1.25, alpha = 1)+
  geom_point(shape = 21,size = 2.25, alpha = .6) +
  theme_linedraw() +
  scale_x_continuous(limits = limits_gen_map_x, breaks=breaks_gen_map_x) +
  scale_y_continuous(limits = limits_gen_map_y, breaks=breaks_gen_map_y) +
  theme_MDR_cartography() +
  ...
  guides(fill = guide_legend(show = FALSE) ) + 
coord_fixed() +    
    geom_label(inherit.aes=FALSE, size = 6, aes(x = -0.45295, y = 1.456,label=paste("A"))) +
scale_fill_manual(values=c("#4DAF4A", "#377EB8", "#E41A1C", "#984EA3")) #+
  #annotate("rect", xmin = limits_gen_map_x_cluster[1], xmax = limits_gen_map_x_cluster[2], ymin = limits_gen_map_y_cluster[1], ymax = limits_gen_map_y_cluster[2],
  #colour = "black")
...
In this example, there are several positions which differentiate clusters 1 and 2. This is a difficult case as to what to mark them, as though they are strongly differentiated, cluster 1 has only 1 PBP-type and cluster 2 has only three (< 5 in total). This may not be such as a problem, if not for the fact that the three PBP-types of cluster 2 only have one isolate each, and so there is not much confidence that this represents their actual position on the map. Because of this, I have marked these positions as weak evidence only, in order to remain conservative. However, notably, PBP2X I371T comes out here and also strongly below for the comparison between clusters 1 and 3.
...
cluster_1_cluster_2_difference <- cluster_diff_subs %>% select(Location, AA_max, cluster_1_cluster_2_max) %>%
  filter(cluster_1_cluster_2_max >= .999)
...
cluster_12_diff <- left_join(cluster_12, select(cluster_1_cluster_2_difference, LABID, Profile_clusters_1_2), by = c("LABID")) %>% ungroup()
...
C <- ggplot(filter(cluster_12_diff, phen_cluster == 2 | phen_cluster == 1), aes(x=G1,y=G2, fill = Profile_clusters_1_2)) +
    geom_point(data = select(cluster_12_diff, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
...
Cluster_substitutions$Genetic_cluster <- "12"
...
In this case, there is one location: PBP2X_I371, where 100% of the isolates in cluster 2 have isoleucine at this position, whereas 100% of isolates of cluster 1 have Threonine at this position. The phenotypic difference between the two clusters can then be attributed to the variation at this position. Notably, this position was also picked out in the 'Identifying single substitutions' analysis, where it generated a phenotypic distance of 4.43. 
...
cluster_12_diff <- left_join(cluster_12, select(cluster_1_cluster_3_difference, LABID, Profile_clusters_1_3), by = c("LABID")) 



cluster_diff_subs$Position <- str_replace_all(cluster_diff_subs$Location, "PBP", "")
cluster_diff_subs$Position <- str_replace_all(cluster_diff_subs$Position, "_", "-")

### Identifying informative isolates --- cluster 1 to 1
A <- ggplot(filter(cluster_diff_subs, cluster_1_cluster_3_max > 0.06), aes(x = reorder(Position, cluster_1_cluster_3_max, FUN =max), y = cluster_1_cluster_3_max, group = 1)) +
  geom_line(color='black') +
  geom_point(size = 3, shape = 21, color='black', fill = "#377EB8", alpha = 0.8) +
  theme_bw() +
  theme(axis.title=element_text(size=12), 
        ...
        axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  theme(legend.position = "none") +
      geom_label(inherit.aes=FALSE, size = 6, aes(x = "2B-M448", y = .95,label=paste("D"))) +

  xlab("PBP position") +
  ylab("AA freq. change (%)") 


cluster_1_cluster_3_difference <- cluster_1_cluster_3_difference %>% mutate(PBP2x_371 = Profile_clusters_1_3)

B <- ggplot(filter(cluster_1_cluster_3_difference, phen_cluster == 1 | phen_cluster == 3), 
       aes(x=D1,y=D2, fill = PBP2x_371, colour= PBP2x_371)) +
   geom_point(data = filter(cluster_1_cluster_3_difference, phen_cluster != 1 | phen_cluster != 3 | phen_cluster != is.na(NA)), 
    ...
    geom_point(data = cluster_1_cluster_3_difference, 
             colour = "#969696", fill = "#969696", shape = 16, size = 2, alpha = 1) +
    geom_point(shape = 16,size = 2, alpha = 1, stroke = 1) +
   geom_point(data = filter(cluster_1_cluster_3_difference, phen_cluster == 1 | phen_cluster == 3), aes(x=x_centroid,y=y_centroid, colour = PBP2x_371), shape = 4, size = 2.25, stroke = 1.25, alpha = 1) +
  theme_linedraw() +
  scale_x_continuous(limits = limits_phen_map_x, breaks=breaks_phen_map_x) +
  scale_y_continuous(limits = limits_phen_map_y, breaks=breaks_phen_map_y) +
  theme_MDR_cartography() +
   ...
   geom_label(inherit.aes=FALSE, size = 3, aes(x = 3.798163, y = 5.697245,label=paste("1"))) +
  geom_label(inherit.aes=FALSE, size = 3, aes(x = 8.925684, y = 8.117218,label=paste("3"))) +
    geom_label(inherit.aes=FALSE, size = 6, aes(x = -1.068912, y = 7.607179,label=paste("E"))) +
coord_fixed() +    
  scale_fill_manual(values=c("#377EB8", "#E41A1C" ))+
  scale_colour_manual(values=c("#377EB8", "#E41A1C" ))
...
### Identifying informative isolates --- cluster 1 to 1
A <- ggplot(filter(cluster_diff_subs, cluster_1_cluster_3_max >= 0.06), aes(x = reorder(Location, cluster_1_cluster_3_max, FUN =max), y = cluster_1_cluster_3_max, group = 1)) +
  geom_line(color='black') +
  geom_point(size = 3, shape = 21, color='black', fill = "#377EB8", alpha = 0.8) +
  theme_bw() +
  theme(axis.title=element_text(size=12), 
...
B <- ggplot(filter(cluster_1_cluster_3_difference, phen_cluster == 3 | phen_cluster == 1), 
       aes(x=D1,y=D2, fill = Profile_clusters_1_3, colour= Profile_clusters_1_3)) +
   geom_point(data = filter(cluster_1_cluster_3_difference, phen_cluster != 3 | phen_cluster != 1 | phen_cluster != is.na(NA)), 
             colour = "black", fill = "#808080", shape = 21, size = 3, alpha = 1) +
    geom_point(data = cluster_1_cluster_3_difference, 
             colour = "#969696", fill = "#969696", shape = 16, size = 2, alpha = 1) +
    geom_point(shape = 16,size = 2, alpha = 1, stroke = 1) +
   geom_point(data = filter(cluster_1_cluster_3_difference, phen_cluster == 3 | phen_cluster == 1), aes(x=x_centroid,y=y_centroid), shape = 21, size = 5, alpha = 1, colour = "black") +
  theme_linedraw() +
  scale_x_continuous(limits = limits_phen_map_x_cluster, breaks=breaks_phen_map_x_cluster) +
  scale_y_continuous(limits = limits_phen_map_y_cluster, breaks=breaks_phen_map_y_cluster) +
  theme_MDR_cartography() +
...
C <- ggplot(filter(cluster_12_diff, phen_cluster == 3 | phen_cluster == 1), aes(x=G1,y=G2, fill = Profile_clusters_1_3)) +
    geom_point(data = select(cluster_12_diff, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
...
In this example, there are several positions which differentiate clusters 1 and 4. I have marked these positions as strong evidence. 

```{r, echo = F}


cluster_1_cluster_4_difference <- cluster_diff_subs %>% select(Location, AA_max, cluster_1_cluster_4_max) %>%
  filter(cluster_1_cluster_4_max >= .999)
...
cluster_12_diff <- left_join(cluster_12, select(cluster_1_cluster_4_difference, LABID, Profile_clusters_1_4), by = c("LABID")) %>% ungroup()



### Identifying informative isolates --- cluster 1 to 1
A <- ggplot(filter(cluster_diff_subs, cluster_1_cluster_4_max >= 0.5), aes(x = reorder(Location, cluster_1_cluster_4_max, FUN =max), y = cluster_1_cluster_4_max, group = 1)) +
  geom_line(color='black') +
  geom_point(size = 3, shape = 21, color='black', fill = "#377EB8", alpha = 0.8) +
  theme_bw() +
  theme(axis.title=element_text(size=12), 
...
C <- ggplot(filter(cluster_12_diff, phen_cluster == 1 | phen_cluster == 4), aes(x=G1,y=G2, fill = Profile_clusters_1_4)) +
    geom_point(data = select(cluster_12_diff, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
...
Here, there were no substitutions which fully segregated between clusters. While there were several which were strongly associated, there was only one isolate for each PBP-type for cluster 2, and these varied considerably within pbp-types in their phenotypic positions. Consequently, I marked the substitutions which differentiated as above 0.95 as having weak evidence of generating change. 

```{r, echo = F}

cluster_2_cluster_3_difference <- cluster_diff_subs %>% select(Location, AA_max, cluster_2_cluster_3_max) %>%
  filter(cluster_2_cluster_3_max >= .95)
...
cluster_12_diff <- left_join(cluster_12, select(cluster_2_cluster_3_difference, LABID, Profile_clusters_2_3), by = c("LABID")) %>% ungroup()
...
C <- ggplot(filter(cluster_12_diff, phen_cluster == 2 | phen_cluster == 3), aes(x=G1,y=G2, fill = Profile_clusters_2_3)) +
    geom_point(data = select(cluster_12_diff, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
...
Here, 4 positions were strongly associated with phenotypic change between clusters 2 and 4. Because the 3 PBPtypes of cluster 2 only had a single isolate each, this only generates weak evidence for each of these substitutions. Importantly, there was also another substitutions PBP2x M400, which did not segregate fully. I made of note of this position for inclusion in the final table as having weak evidence. 

```{r, echo = F}

cluster_2_cluster_4_difference <- cluster_diff_subs %>% select(Location, AA_max, cluster_2_cluster_4_max) %>%
  filter(cluster_2_cluster_4_max >= .999)
...
cluster_12_diff <- left_join(cluster_12, select(cluster_2_cluster_4_difference, LABID, Profile_clusters_2_4), by = c("LABID")) %>% ungroup()
...
C <- ggplot(filter(cluster_12_diff, phen_cluster == 2 | phen_cluster == 4), aes(x=G1,y=G2, fill = Profile_clusters_2_4)) +
    geom_point(data = select(cluster_12_diff, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
...
temp <- as.data.frame(colnames(cluster_2_cluster_4_difference[,11:ncol(cluster_2_cluster_4_difference)]))
colnames(temp) <- "Location"

temp$Genetic_cluster <- "12"
temp$Phenotypic_cluster <- "2_4"
temp$Confidence <- "Weak"
...
In this last comparison, there was a clear difference in four amino acid positions between clusters 3 and 4. Notably, there was also another position PBP2X M400, which was also strongly associated, but not 100%. This position also came out above in the cluster 2-4 comparison. 
...
cluster_3_cluster_4_difference <- cluster_diff_subs %>% select(Location, AA_max, cluster_3_cluster_4_max) %>%
  filter(cluster_3_cluster_4_max >= .94)
...
cluster_12_diff <- left_join(cluster_12, select(cluster_3_cluster_4_difference, LABID, Profile_clusters_3_4), by = c("LABID")) %>% ungroup()
...
### Identifying informative isolates --- cluster 1 to 1
A <- ggplot(filter(cluster_diff_subs, cluster_3_cluster_4_max > 0.9), aes(x = reorder(Position, cluster_3_cluster_4_max, FUN =max), y = cluster_3_cluster_4_max, group = 1)) +
  geom_line(color='black') +
  geom_point(size = 3, shape = 21, color='black', fill = "#377EB8", alpha = 0.8) +
  theme_bw() +
  theme(axis.title=element_text(size=12), 
        ...
        axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  theme(legend.position = "none") +
      geom_label(inherit.aes=FALSE, size = 6, aes(x = "2X-D506", y = .995,label=paste("F"))) +
  xlab("PBP position") +
  
  ylab("AA freq. change (%)") 


cluster_3_cluster_4_difference <- cluster_3_cluster_4_difference %>% mutate(AA_Combination = Profile_clusters_3_4)

B <- ggplot(filter(cluster_3_cluster_4_difference, phen_cluster == 3 | phen_cluster == 4), 
       aes(x=D1,y=D2, fill = AA_Combination, colour= AA_Combination)) +
   geom_point(data = filter(cluster_3_cluster_4_difference, phen_cluster != 3 | phen_cluster != 4 | phen_cluster != is.na(NA)), 
             colour = "black", fill = "#808080", shape = 21, size = 3, alpha = 1) +
    geom_point(data = cluster_3_cluster_4_difference, 
             colour = "#969696", fill = "#969696", shape = 16, size = 2, alpha = 1) +
    geom_point(shape = 16,size = 2, alpha = 1, stroke = 1) +
   geom_point(data = filter(cluster_3_cluster_4_difference, phen_cluster == 3 | phen_cluster == 4), aes(x=x_centroid,y=y_centroid, colour = AA_Combination), shape = 4, size = 2.25, stroke = 1.25, alpha = 1) +
  theme_linedraw() +
  scale_x_continuous(limits = limits_phen_map_x, breaks=breaks_phen_map_x) +
  scale_y_continuous(limits = limits_phen_map_y, breaks=breaks_phen_map_y) +
  theme_MDR_cartography() +
  geom_label(inherit.aes=FALSE, size = 3, aes(x = 8.925684, y = 8.117218,label=paste("3"))) +
    geom_label(inherit.aes=FALSE, size = 3, aes(x = 11.88283, y = 3.5757,label=paste("4"))) +
  geom_label(inherit.aes=FALSE, size = 6, aes(x = -1.068912, y = 7.607179,label=paste("F"))) +
...
C <- ggplot(filter(cluster_12_diff, phen_cluster == 3 | phen_cluster == 4), aes(x=G1,y=G2, fill = Profile_clusters_3_4)) +
    geom_point(data = select(cluster_12_diff, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
...
C
A
#ggsave("pneumo_gen_clust_12_phenclusters_phen_compAA_comb.jpg")



temp <- temp %>%
  left_join(select(cluster_diff_subs, Location, cluster_3_cluster_4_max), by = "Location") %>%
  mutate(
    Genetic_cluster = "12",
    Phenotypic_cluster = "3_4",
    Confidence = case_when(
      cluster_3_cluster_4_max == 1 ~ "Strong",
      cluster_3_cluster_4_max >= 0.94 ~ "Weak",
      TRUE ~ "Ignore"
    )
  ) %>%
  filter(Confidence != "Ignore") %>%
  select(Location, Genetic_cluster, Phenotypic_cluster, Confidence)

Cluster_substitutions <- bind_rows(Cluster_substitutions, temp)

```


# Summary
In total, 33 PBP changes were associated with the phenotypic cluster differences in this subset of isolates. 

```{r, echo =F} 

Potential_substitutions <- Cluster_substitutions %>%
  filter(Location != "AA_Combination",
         Location != "PBP2x_371")

nrow(Potential_substitutions)
setwd(AMRC_PNEUMO_ANALYSIS_DIR)

write.csv(Potential_substitutions, file = "Sig_AA_subs_Cluster_gen12_S.pneumo.csv", row.names=FALSE)
...
These occurred at 15 distinct locations:
