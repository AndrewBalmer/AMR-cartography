...
ggplot(filter(AA_matrix_phenotype, gen_cluster == 11), aes(x=G1,y=G2, fill = gen_cluster)) +
...
cluster_11 <- filter(AA_matrix_phenotype, gen_cluster == 11)



## Pull out the AA sequences for the isolates
PBP_positions <- cluster_11 %>% 
  ungroup() %>%
  select(starts_with("PBP"), phen_cluster) %>%
  select(-PBP_type)

    
limits_phen_map_x_cluster <- c(min(cluster_11$D1, na.rm = T), 
                       max(cluster_11$D1, na.rm = T))
limits_phen_map_y_cluster <- c(min(cluster_11$D2, na.rm = T), 
                       max(cluster_11$D2, na.rm = T))

breaks_phen_map_x_cluster <- seq(min(cluster_11$D1, na.rm = T) - (1), 
                         max(cluster_11$D1, na.rm = T) + (1), 1)
breaks_phen_map_y_cluster <- seq(min(cluster_11$D2, na.rm = T) - (1), 
                         max(cluster_11$D2, na.rm = T) + (1), 1)


limits_gen_map_x_cluster <- c(min(cluster_11$G1, na.rm = T) - (gen_slope * 5), 
                      max(cluster_11$G1, na.rm = T) + (gen_slope * 5))
limits_gen_map_y_cluster <- c(min(cluster_11$G2, na.rm = T) - (gen_slope * 5), 
                      max(cluster_11$G2, na.rm = T)+ (gen_slope * 5))

breaks_gen_map_x_cluster <- seq(min(cluster_11$G1, na.rm = T)- (gen_slope * 10), 
                        max(cluster_11$G1, na.rm = T) + (gen_slope * 10), gen_slope * 10)
breaks_gen_map_y_cluster <- seq(min(cluster_11$G2, na.rm = T)- (gen_slope * 10), 
                        max(cluster_11$G2, na.rm = T) + (gen_slope * 10), gen_slope * 10)

ggplot(filter(cluster_11, gen_cluster == 11), aes(x=G1,y=G2, fill = gen_cluster)) +
    geom_point(data = select(all_isolates, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
...
Here, I have plotted the isolates of cluster 11 on the phenotype map. Each colour represents a different genotype. The CDC uses a PBP typing scheme, where the genotype of each of the three PBPs is given a number. The labels represent the particular genotype combination for each of the three proteins.

```{r, echo = F}


ggplot(filter(cluster_11, gen_cluster == 11), aes(x=x_centroid,y=y_centroid, fill = PBP_type)) +
  geom_point(data = select(all_isolates, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
...
ggplot(filter(cluster_11, gen_cluster == 11), aes(x=G1,y=G2, fill = PBP_type)) +
    geom_point(data = select(all_isolates, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
  geom_point(shape = 21,size = 5, alpha = .6) +
  theme_linedraw() +
...
Here I have tried to classify the isolates based on their phenotype, however this is difficult in some cases given outliers. I have therefore calculated the median centroid for each genotype on the map, and marked these as a series of crosses (X). The crosses do not really fall into discrete groups, but are placed into two groupings based on the results of the scree plot, as shown below. 

```{r, echo =FALSE}

ggplot(filter(AA_matrix_phenotype, gen_cluster == 11), aes(x=D1,y=D2, fill = PBP_type)) +
  geom_point(data = select(AA_matrix_phenotype, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
  geom_point(shape = 21,size = 4, alpha = 1) +
  geom_point(cluster_11, mapping = aes(x_centroid, y_centroid, colour = "#000000"),
...
I then used the hierarchical clustering algorithm on the centroid positions to pick out 2 regions which have differing phenotypes within this group of low PBP genetic diversity. Here, the centroids and the isolates which belong to that centroid are coloured by their phenotype cluster. It should be noted that in this example, the isolates do not obviously fall into separate phenotypic clusters, and so the method is unlikely to be able to pick out substitutions associated with a change in phenotype.

```{r, echo =F}

ggplot(filter(cluster_11), aes(x=D1,y=D2, fill = phen_cluster)) +
  geom_point(data = select(all_isolates, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
    geom_point(shape = 21,size = 4, alpha = 1) +
  geom_point(cluster_11, mapping = aes(x_centroid, y_centroid, colour = phen_cluster),
...
In this example, there are no substitutions which clearly differentiate the two clusters, because as stated above, there is substantial overlap in distribution between the two groupings. 
...
cluster_11_diff <- left_join(cluster_11, select(cluster_1_cluster_2_difference, LABID, Profile_clusters_1_2), by = c("LABID")) %>% ungroup()
...
C <- ggplot(filter(cluster_11_diff, phen_cluster == 2 | phen_cluster == 1), aes(x=G1,y=G2, fill = Profile_clusters_1_2)) +
    geom_point(data = select(cluster_11_diff, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
...
# Summary
In total, no PBP locations were strongly associated with the phenotypic cluster differences in this subset of isolates. This is likely because the two clusters are phenotypically very similar to one another and many of the isolates and pbp types are overlapping.
