...
ggplot(filter(AA_matrix_phenotype, gen_cluster == 9), aes(x=G1,y=G2, fill = gen_cluster)) +
...
cluster_9 <- filter(AA_matrix_phenotype, gen_cluster == 9)


## Pull out the AA sequences for the isolates
PBP_positions <- cluster_9 %>% 
  ungroup() %>%
  select(starts_with("PBP"), phen_cluster) %>%
  select(-PBP_type)

    
limits_phen_map_x_cluster <- c(min(cluster_9$D1, na.rm = T), 
                       max(cluster_9$D1, na.rm = T))
limits_phen_map_y_cluster <- c(min(cluster_9$D2, na.rm = T), 
                       max(cluster_9$D2, na.rm = T))

breaks_phen_map_x_cluster <- seq(min(cluster_9$D1, na.rm = T) - (1), 
                         max(cluster_9$D1, na.rm = T) + (1), 1)
breaks_phen_map_y_cluster <- seq(min(cluster_9$D2, na.rm = T) - (1), 
                         max(cluster_9$D2, na.rm = T) + (1), 1)


limits_gen_map_x_cluster <- c(min(cluster_9$G1, na.rm = T) - (gen_slope * 5), 
                      max(cluster_9$G1, na.rm = T) + (gen_slope * 5))
limits_gen_map_y_cluster <- c(min(cluster_9$G2, na.rm = T) - (gen_slope * 5), 
                      max(cluster_9$G2, na.rm = T)+ (gen_slope * 5))

breaks_gen_map_x_cluster <- seq(min(cluster_9$G1, na.rm = T)- (gen_slope * 10), 
                        max(cluster_9$G1, na.rm = T) + (gen_slope * 10), gen_slope * 10)
breaks_gen_map_y_cluster <- seq(min(cluster_9$G2, na.rm = T)- (gen_slope * 10), 
                        max(cluster_9$G2, na.rm = T) + (gen_slope * 10), gen_slope * 10)

ggplot(filter(cluster_9, gen_cluster == 9), aes(x=G1,y=G2, fill = gen_cluster)) +
    geom_point(data = select(all_isolates, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
...
ggplot(filter(cluster_9, gen_cluster == 9), aes(x=x_centroid,y=y_centroid, fill = PBP_type)) +
  geom_point(data = select(all_isolates, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
...
ggplot(filter(cluster_9, gen_cluster == 9), aes(x=G1,y=G2, fill = PBP_type)) +
    geom_point(data = select(all_isolates, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
  geom_point(shape = 21,size = 5, alpha = .6) +
  theme_linedraw() +
...
Here I have tried to classify the isolates based on their phenotype, however this is difficult in some cases given outliers. I have therefore calculated the median centroid for each genotype on the map, and marked these as a series of crosses (X). The crosses fall roughly into 2 groupings, as shown below. Although the isolates could have been split into 3 groups, two clusters were used in total. This is because the isolates overlapped significantly in phenotypes, most variation could be captured using two clusters, and splitting them into three clusters did not improve the resolution of picking out substitutions between the isolates.

```{r, echo =FALSE}

ggplot(filter(AA_matrix_phenotype, gen_cluster == 9), aes(x=D1,y=D2, fill = PBP_type)) +
  geom_point(data = select(AA_matrix_phenotype, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
  geom_point(shape = 21,size = 4, alpha = 1) +
  geom_point(cluster_9, mapping = aes(x_centroid, y_centroid, colour = "#000000"),
...
ggplot(filter(cluster_9), aes(x=D1,y=D2, fill = phen_cluster)) +
  geom_point(data = select(all_isolates, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
    geom_point(shape = 21,size = 4, alpha = 1) +
  geom_point(cluster_9, mapping = aes(x_centroid, y_centroid, colour = phen_cluster),
...
In this single comparison, there was a clear difference in amino acid at many positions between clusters 1 and 2. However, there was only 1 PBP-types in cluster 1, and this was only represented by a single isolate.  This suggests these amino acids are only being picked out by chance due to the very low number of isolates, and so were not marked as weak or strong evidence. 
...
cluster_9_diff <- left_join(cluster_9, select(cluster_1_cluster_2_difference, LABID, Profile_clusters_1_2), by = c("LABID")) %>% ungroup()
...
C <- ggplot(filter(cluster_9_diff, phen_cluster == 2 | phen_cluster == 1), aes(x=G1,y=G2, fill = Profile_clusters_1_2)) +
    geom_point(data = select(cluster_9_diff, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
...
# Summary
In total, no PBP locations were strongly associated with the phenotypic cluster differences in this subset of isolates. 
