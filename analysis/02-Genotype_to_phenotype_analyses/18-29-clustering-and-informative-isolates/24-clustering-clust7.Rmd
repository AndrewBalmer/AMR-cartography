...
ggplot(filter(AA_matrix_phenotype, gen_cluster == 7), aes(x=G1,y=G2, fill = gen_cluster)) +
...
PBPclustering <- AA_matrix_phenotype %>% 
  filter(gen_cluster == 7) %>% 
  distinct(PBP_type, .keep_all = TRUE) %>% 
  ungroup()

# Compute hierarchical clustering on MDS axes
phen_cluster_MDS <- HCPC(select(PBPclustering, x_centroid, y_centroid), nb.clust = 2, consol = T, iter.max = 500, graph = T, order = T)
...
cluster_7 <- filter(AA_matrix_phenotype, gen_cluster == 7)



## Pull out the AA sequences for the isolates
PBP_positions <- cluster_7 %>% 
  ungroup() %>%
  select(starts_with("PBP"), phen_cluster) %>%
  select(-PBP_type)

    
limits_phen_map_x_cluster <- c(min(cluster_7$D1, na.rm = T), 
                       max(cluster_7$D1, na.rm = T))
limits_phen_map_y_cluster <- c(min(cluster_7$D2, na.rm = T), 
                       max(cluster_7$D2, na.rm = T))

breaks_phen_map_x_cluster <- seq(min(cluster_7$D1, na.rm = T) - (1), 
                         max(cluster_7$D1, na.rm = T) + (1), 1)
breaks_phen_map_y_cluster <- seq(min(cluster_7$D2, na.rm = T) - (1), 
                         max(cluster_7$D2, na.rm = T) + (1), 1)


limits_gen_map_x_cluster <- c(min(cluster_7$G1, na.rm = T) - (gen_slope * 5), 
                      max(cluster_7$G1, na.rm = T) + (gen_slope * 5))
limits_gen_map_y_cluster <- c(min(cluster_7$G2, na.rm = T) - (gen_slope * 5), 
                      max(cluster_7$G2, na.rm = T)+ (gen_slope * 5))

breaks_gen_map_x_cluster <- seq(min(cluster_7$G1, na.rm = T)- (gen_slope * 10), 
                        max(cluster_7$G1, na.rm = T) + (gen_slope * 10), gen_slope * 10)
breaks_gen_map_y_cluster <- seq(min(cluster_7$G2, na.rm = T)- (gen_slope * 10), 
                        max(cluster_7$G2, na.rm = T) + (gen_slope * 10), gen_slope * 10)

ggplot(filter(cluster_7, gen_cluster == 7), aes(x=G1,y=G2, fill = gen_cluster)) +
    geom_point(data = select(all_isolates, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
...
Here, I have plotted the isolates of cluster 7 on the phenotype map. Each colour represents a different genotype. The CDC uses a PBP typing scheme, where the genotype of each of the three PBPs is given a number. The labels represent the particular genotype combination for each of the three proteins.


```{r, echo = F}


ggplot(filter(cluster_7, gen_cluster == 7), aes(x=x_centroid,y=y_centroid, fill = PBP_type)) +
  geom_point(data = select(all_isolates, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
...
ggplot(filter(cluster_7, gen_cluster == 7), aes(x=G1,y=G2, fill = PBP_type)) +
    geom_point(data = select(all_isolates, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
  geom_point(shape = 21,size = 5, alpha = .6) +
  theme_linedraw() +
...
Here I have tried to classify the isolates based on their phenotype, however this is difficult in some cases given outliers. I have therefore calculated the median centroid for each genotype on the map, and marked these as a series of crosses (X). The crosses fall roughly into two  groupings, as shown below.

```{r, echo =FALSE}

ggplot(filter(AA_matrix_phenotype, gen_cluster == 7), aes(x=D1,y=D2, fill = PBP_type)) +
  geom_point(data = select(AA_matrix_phenotype, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
  geom_point(shape = 21,size = 4, alpha = 1) +
  geom_point(cluster_7, mapping = aes(x_centroid, y_centroid, colour = "#000000"),
...
I then used the hierarchical clustering algorithm on the centroid positions to pick out 2 regions which have strongly differing phenotypes within this group of low PBP genetic diversity. Here, the centroids and the isolates which belong to that centroid are coloured by their phenotype cluster. Notably, the scree plot above showed a strong elbow at 3 clusters. However, as the two bottom left centroids were both close together, and one had a very low number of isolates, I decided to combine them into a single cluster for the purposes of this analysis. 

```{r, echo =F}

ggplot(filter(cluster_7), aes(x=D1,y=D2, fill = phen_cluster)) +
  geom_point(data = select(all_isolates, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
    geom_point(shape = 21,size = 4, alpha = 1) +
  geom_point(cluster_7, mapping = aes(x_centroid, y_centroid, colour = phen_cluster),
...
Potential “cluster-difference” amino acid substitutions were identified using the following method. A loci variant of X to Y at location L was considered a potential “cluster-difference” variant between clusters A and B if all or all but one isolates in cluster A have variant X at location L, and all or all but one isolates in cluster B have variant Y at location L. 

## Example 1 --- comparing phenotype clusters 1 and 2

In this comparison, there was a clear difference in amino acid at 8 positions between clusters 1 and 2. However 7 out of 8 of these several other positions which differentiated strongly, they were not 100%, due to the single outlier PBP type in cluster 2. 7 of these 8 were therefore marked as having 'weak' evidence, whereas the one which segregated fully was classed at 'Strong'. 

Notably however, many of these 7 were found to be strongly significant in the multivariate linear mixed model analysis. 
...
cluster_1_cluster_2_difference <- cluster_diff_subs %>% select(Location, AA_max, cluster_1_cluster_2_max) %>%
  filter(cluster_1_cluster_2_max >= .97)
...
cluster_7_diff <- left_join(cluster_7, select(cluster_1_cluster_2_difference, LABID, Profile_clusters_1_2), by = c("LABID")) %>% ungroup()



### Identifying informative isolates --- cluster 2 to 1
A <- ggplot(filter(cluster_diff_subs, cluster_1_cluster_2_max >= 0.8), aes(x = reorder(Location, cluster_1_cluster_2_max, FUN =max), y = cluster_1_cluster_2_max, group = 1)) +
...
C <- ggplot(filter(cluster_7_diff, phen_cluster == 2 | phen_cluster == 1), aes(x=G1,y=G2, fill = Profile_clusters_1_2)) +
    geom_point(data = select(cluster_7_diff, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
...
Cluster_substitutions <- colnames(cluster_1_cluster_2_difference[,11:ncol(cluster_1_cluster_2_difference)])

Cluster_substitutions <- as.data.frame(Cluster_substitutions)
colnames(Cluster_substitutions) <- "Location"

Cluster_substitutions <- Cluster_substitutions %>%
  left_join(select(cluster_diff_subs, Location, cluster_1_cluster_2_max), by = "Location") %>%
  mutate(
    Genetic_cluster = "7",
    Phenotypic_cluster = "1_2",
    Confidence = case_when(
      cluster_1_cluster_2_max == 1 ~ "Strong",
      cluster_1_cluster_2_max >= 0.97 ~ "Weak",
      TRUE ~ "Ignore"  # Or filter these out if undesired
    )
  ) %>%
...
ggplot(filter(cluster_1_cluster_2_difference, phen_cluster == 2 | phen_cluster == 1), 
       aes(x=D1,y=D2, fill = Profile_clusters_1_2, colour= Profile_clusters_1_2)) +
   geom_point(data = filter(cluster_1_cluster_2_difference, phen_cluster != 2 | phen_cluster != 1 | phen_cluster != is.na(NA)), 
             colour = "black", fill = "#808080", shape = 21, size = 3, alpha = 1) +
    geom_point(data = cluster_1_cluster_2_difference, 
...
# Summary
In total, 8 PBP locations were associated with the phenotypic cluster differences in this subset of isolates, 1 of which strongly associated. All 8 were located in the PBP2x protein. 
...
write.csv(Potential_substitutions, file = "Sig_AA_subs_Cluster_gen7_S.pneumo.csv", row.names=FALSE)
```
