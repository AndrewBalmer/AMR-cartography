...
ggplot(filter(AA_matrix_phenotype, gen_cluster == 5), aes(x=G1,y=G2, fill = gen_cluster)) +
...
cluster_5 <- filter(AA_matrix_phenotype, gen_cluster == 5)
...
ggplot(filter(cluster_5, gen_cluster == 5), aes(x=G1,y=G2, fill = gen_cluster)) +
    geom_point(data = select(all_isolates, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
...
Here, I have plotted the isolates of cluster 5 on the phenotype map. Each colour represents a different genotype. The CDC uses a PBP typing scheme, where the genotype of each of the three PBPs is given a number. The labels represent the particular genotype combination for each of the three proteins.

```{r, echo = F}


ggplot(filter(cluster_5, gen_cluster == 5), aes(x=x_centroid,y=y_centroid, fill = PBP_type)) +
  geom_point(data = select(all_isolates, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
...
ggplot(filter(cluster_5, gen_cluster == 5), aes(x=G1,y=G2, fill = PBP_type)) +
    geom_point(data = select(all_isolates, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
  geom_point(shape = 21,size = 5, alpha = .6) +
  theme_linedraw() +
...
Here I have tried to classify the isolates based on their phenotype, however this is difficult in some cases given outliers. I have therefore calculated the median centroid for each genotype on the map, and marked these as a series of crosses (X). The crosses fall roughly into three groupings, as shown below. Although the results of the scree plot suggest four grouping may be a better choice, the isolates appear grouped into three major groups, with cluster 2 having a subset of isolates which are slightly different. Splitting the isolates into 3 groups was the better choice, as it was more biologically interpretable. In this case, this is because it provided additional PBP-types with which to compare clusters 1 and 2. 

```{r, echo =FALSE}

ggplot(filter(AA_matrix_phenotype, gen_cluster == 5), aes(x=D1,y=D2, fill = PBP_type)) +
...
In this case there were 6 substitutions which fully segregates between clusters 1 and 2 (>0.999). 
...
A <- ggplot(filter(cluster_diff_subs, cluster_1_cluster_2_max >= 0.7), aes(x = reorder(Location, cluster_1_cluster_2_max, FUN =max), y = cluster_1_cluster_2_max, group = 1)) +
...
Cluster_substitutions$Genetic_cluster <- "5"
...
In this case there were many substitutions which fully segregates between clusters 1 and 3 (>0.999). however, as there is only one PBP-type in cluster 3, and there was only one of these isolates, these substitutions were marked as weak evidence only.
...
cluster_5_diff <- left_join(cluster_5, select(cluster_1_cluster_3_difference, LABID, Profile_clusters_1_3), by = c("LABID")) %>% ungroup()
...
C <- ggplot(filter(cluster_5_diff, phen_cluster == 1 | phen_cluster == 3), aes(x=G1,y=G2, fill = Profile_clusters_1_3)) +
    geom_point(data = select(cluster_5_diff, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
...
In this case there were many substitutions which fully segregates between clusters 1 and 3 (>0.999). however, as there is only one PBP-type in cluster 3, and there was only one isolate of this PBP-type, these substitutions were marked as weak evidence only.
...
cluster_5_diff <- left_join(cluster_5, select(cluster_2_cluster_3_difference, LABID, Profile_clusters_2_3), by = c("LABID")) %>% ungroup()



### Identifying informative isolates --- cluster 2 to 1
A <- ggplot(filter(cluster_diff_subs, cluster_2_cluster_3_max >= 0.8), aes(x = reorder(Location, cluster_2_cluster_3_max, FUN =max), y = cluster_2_cluster_3_max, group = 1)) +
  geom_line(color='black') +
  geom_point(size = 3, shape = 21, color='black', fill = "#377EB8", alpha = 0.8) +
  theme_bw() +
  theme(axis.title=element_text(size=12), 
...
C <- ggplot(filter(cluster_5_diff, phen_cluster == 2 | phen_cluster == 3), aes(x=G1,y=G2, fill = Profile_clusters_2_3)) +
    geom_point(data = select(cluster_5_diff, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
...
# Summary
In total, 51 distinct PBP locations were associated with the phenotypic cluster differences in this subset of isolates, although only 6 had strong evidence of generating an effect. All 6 were located in the PBP2x protein. 
...
write.csv(Potential_substitutions, file = "Sig_AA_subs_Cluster_gen5_S.pneumo.csv", row.names=FALSE)
