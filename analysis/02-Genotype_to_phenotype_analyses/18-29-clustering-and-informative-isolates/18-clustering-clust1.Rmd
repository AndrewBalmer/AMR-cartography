...
Most of the variation in cluster 1 could be explained in 2 clusters, as indicated by the sharp elbow on the scree plot at 2 clusters. However, there was very little phenotypic variation within this group generally, making it difficult to pick out substitutions which vary between groups. Similarly, there was a low number of PBP-types, and the low number of isolates within each PBP-type. This is reflected in the output of this file, as no substitutions were clearly associated with the phenotypic distance between the isolates in this group. 
...
cluster_1 <- filter(AA_matrix_phenotype, gen_cluster == 1)



## Pull out the AA sequences for the isolates
PBP_positions <- cluster_1 %>% 
  ungroup() %>%
  select(starts_with("PBP"), phen_cluster) %>%
  select(-PBP_type)

    
limits_phen_map_x_cluster <- c(min(cluster_1$D1, na.rm = T), 
                       max(cluster_1$D1, na.rm = T))
limits_phen_map_y_cluster <- c(min(cluster_1$D2, na.rm = T), 
                       max(cluster_1$D2, na.rm = T))

breaks_phen_map_x_cluster <- seq(min(cluster_1$D1, na.rm = T) - (1), 
                         max(cluster_1$D1, na.rm = T) + (1), 1)
breaks_phen_map_y_cluster <- seq(min(cluster_1$D2, na.rm = T) - (1), 
                         max(cluster_1$D2, na.rm = T) + (1), 1)


limits_gen_map_x_cluster <- c(min(cluster_1$G1, na.rm = T) - (gen_slope * 5), 
                      max(cluster_1$G1, na.rm = T) + (gen_slope * 5))
limits_gen_map_y_cluster <- c(min(cluster_1$G2, na.rm = T) - (gen_slope * 5), 
                      max(cluster_1$G2, na.rm = T)+ (gen_slope * 5))

breaks_gen_map_x_cluster <- seq(min(cluster_1$G1, na.rm = T)- (gen_slope * 10), 
                        max(cluster_1$G1, na.rm = T) + (gen_slope * 10), gen_slope * 10)
breaks_gen_map_y_cluster <- seq(min(cluster_1$G2, na.rm = T)- (gen_slope * 10), 
                        max(cluster_1$G2, na.rm = T) + (gen_slope * 10), gen_slope * 10)

ggplot(filter(cluster_1, gen_cluster == 1 ), aes(x=D1,y=D2, fill = gen_cluster)) +
    geom_point(data = select(all_isolates, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
...
ggplot(filter(cluster_1, gen_cluster == 1 ), aes(x=x_centroid,y=y_centroid, fill = PBP_type)) +
  geom_point(data = select(all_isolates, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
...
ggplot(filter(cluster_1, gen_cluster == 1 ), aes(x=G1,y=G2, fill = PBP_type)) +
    geom_point(data = select(all_isolates, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
  geom_point(shape = 21,size = 5, alpha = .6) +
  theme_linedraw() +
...
Here I have tried to classify the isolates based on their phenotype, however this is difficult in some cases given outliers. I have therefore calculated the median centroid for each genotype on the map, and marked these as a series of crosses (X). In this cluster, this method has limited resolution, because the PBP-types are so similar in phenotype. Because of this, there will be limited information from this cluster. For a better example of the method and its uses, look at the script for cluster 12. 
...
ggplot(filter(AA_matrix_phenotype, gen_cluster == 1 ), aes(x=D1,y=D2, fill = PBP_type)) +
  ...
  geom_point(shape = 21,size = 4, alpha = 1) +
  geom_point(cluster_1, mapping = aes(x_centroid, y_centroid, colour = "#000000"),
...
I then used the hierarchical clustering algorithm on the centroid positions to pick out  regions which have different phenotypes within this group of low PBP genetic diversity. Here, the centroids and the isolates which belong to that centroid are coloured by their phenotype cluster.

```{r, echo =F}

ggplot(filter(cluster_1), aes(x=D1,y=D2, fill = phen_cluster)) +
  geom_point(data = select(all_isolates, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
    geom_point(shape = 21,size = 4, alpha = 1) +
  geom_point(cluster_1, mapping = aes(x_centroid, y_centroid, colour = phen_cluster),
...
As stated above, the limited phenotypic diversity within this cluster gave very limited information on the amino acid differences between clusters, as it is difficult to establish any robust comparison between the groups.
...
cluster_1_diff <- left_join(cluster_1, select(cluster_1_cluster_2_difference, LABID, Profile_clusters_1_2), by = c("LABID")) %>% ungroup()
...
C <- ggplot(filter(cluster_1_diff, phen_cluster == 2 | phen_cluster == 1), aes(x=G1,y=G2, fill = Profile_clusters_1_2)) +
    geom_point(data = select(cluster_1_diff, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
...
As none of the substitutions were clearly associated with the difference between phenotypic clusters. None were marked as being associated with phenotypic change.
