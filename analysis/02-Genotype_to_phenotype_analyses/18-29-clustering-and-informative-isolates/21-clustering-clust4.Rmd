...
ggplot(filter(AA_matrix_phenotype, gen_cluster == 4), aes(x=G1,y=G2, fill = gen_cluster)) +
...
cluster_4 <- filter(AA_matrix_phenotype, gen_cluster == 4)



## Pull out the AA sequences for the isolates
PBP_positions <- cluster_4 %>% 
  ungroup() %>%
  select(starts_with("PBP"), phen_cluster) %>%
  select(-PBP_type)

    
limits_phen_map_x_cluster <- c(min(cluster_4$D1, na.rm = T), 
                       max(cluster_4$D1, na.rm = T))
limits_phen_map_y_cluster <- c(min(cluster_4$D2, na.rm = T), 
                       max(cluster_4$D2, na.rm = T))

breaks_phen_map_x_cluster <- seq(min(cluster_4$D1, na.rm = T) - (slope), 
                         max(cluster_4$D1, na.rm = T) + (slope), slope)
breaks_phen_map_y_cluster <- seq(min(cluster_4$D2, na.rm = T) - (slope), 
                         max(cluster_4$D2, na.rm = T) + (slope), slope)


limits_gen_map_x_cluster <- c(min(cluster_4$G1, na.rm = T) - (gen_slope * 5), 
                      max(cluster_4$G1, na.rm = T) + (gen_slope * 5))
limits_gen_map_y_cluster <- c(min(cluster_4$G2, na.rm = T) - (gen_slope * 5), 
                      max(cluster_4$G2, na.rm = T)+ (gen_slope * 5))

breaks_gen_map_x_cluster <- seq(min(cluster_4$G1, na.rm = T)- (gen_slope * 10), 
                        max(cluster_4$G1, na.rm = T) + (gen_slope * 10), gen_slope * 10)
breaks_gen_map_y_cluster <- seq(min(cluster_4$G2, na.rm = T)- (gen_slope * 10), 
                        max(cluster_4$G2, na.rm = T) + (gen_slope * 10), gen_slope * 10)

ggplot(filter(cluster_4, gen_cluster == 4), aes(x=G1,y=G2, fill = gen_cluster)) +
    geom_point(data = select(all_isolates, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
...
Here, I have plotted the isolates of cluster 4 on the phenotype map. Each colour represents a different genotype. The CDC uses a PBP typing scheme, where the genotype of each of the three PBPs is given a number. The labels represent the particular genotype combination for each of the three proteins.

```{r, echo = F}


ggplot(filter(cluster_4, gen_cluster == 4), aes(x=x_centroid,y=y_centroid, fill = PBP_type)) +
  geom_point(data = select(all_isolates, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
...
ggplot(filter(cluster_4, gen_cluster == 4), aes(x=G1,y=G2, fill = PBP_type)) +
    geom_point(data = select(all_isolates, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
  geom_point(shape = 21,size = 5, alpha = .6) +
  theme_linedraw() +
  scale_x_continuous(limits = limits_gen_map_x_cluster, breaks=breaks_gen_map_x_cluster) +
...
ggplot(filter(AA_matrix_phenotype, gen_cluster == 4), aes(x=D1,y=D2, fill = PBP_type)) +
  geom_point(data = select(AA_matrix_phenotype, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
  geom_point(shape = 21,size = 4, alpha = 1) +
  geom_point(cluster_4, mapping = aes(x_centroid, y_centroid, colour = "#000000"),
...
I then used the hierarchical clustering algorithm on the centroid positions to pick out the 4 regions which have differing phenotypes within this group of low PBP genetic diversity. Here, the centroids and the isolates which belong to that centroid are coloured by their phenotype cluster.

```{r, echo =F}

ggplot(filter(cluster_4), aes(x=D1,y=D2, fill = phen_cluster)) +
  geom_point(data = select(all_isolates, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
    geom_point(shape = 21,size = 4, alpha = 1) +
  geom_point(cluster_4, mapping = aes(x_centroid, y_centroid, colour = phen_cluster),
...
Below is an example of this method. The first plot shows the AA positions of the three PBPs on the X axis. The Y axis shows the absolute proportion change in frequency of an amino acid residue at that position. 

Although there are many substitutions which vary strongly between clusters 1 and 2 here, there is only a single isolate in cluster 1. Therefore, I have marked these as weak evidence only.
...
cluster_1_cluster_2_difference <- cluster_diff_subs %>% select(Location, AA_max, cluster_1_cluster_2_max) %>%
  filter(cluster_1_cluster_2_max >= .99)
...
cluster_4_diff <- left_join(cluster_4, select(cluster_1_cluster_2_difference, LABID, Profile_clusters_1_2), by = c("LABID")) %>% ungroup()
...
C <- ggplot(filter(cluster_4_diff, phen_cluster == 2 | phen_cluster == 1), aes(x=G1,y=G2, fill = Profile_clusters_1_2)) +
    geom_point(data = select(cluster_4_diff, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
...
Cluster_substitutions$Genetic_cluster <- "4"
...
In this second case, there are again several locations which strongly differentiate clusters 1 and 3. However, as before there is only a single isolate in cluster 1. Therefore, I have marked these as weak evidence only.
...
cluster_1_cluster_3_difference <- cluster_diff_subs %>% select(Location, AA_max, cluster_1_cluster_3_max) %>%
  filter(cluster_1_cluster_3_max >= .9)
...
cluster_4_diff <- left_join(cluster_4, select(cluster_1_cluster_3_difference, LABID, Profile_clusters_1_3), by = c("LABID")) %>% ungroup()
...
C <- ggplot(filter(cluster_4_diff, phen_cluster == 1 | phen_cluster == 3), aes(x=G1,y=G2, fill = Profile_clusters_1_3)) +
    geom_point(data = select(cluster_4_diff, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
...
cluster_1_cluster_4_difference <- cluster_diff_subs %>% select(Location, AA_max, cluster_1_cluster_4_max) %>%
  filter(cluster_1_cluster_4_max >= .5)
...
cluster_4_diff <- left_join(cluster_4, select(cluster_1_cluster_4_difference, LABID, Profile_clusters_1_4), by = c("LABID")) %>% ungroup()
...
C <- ggplot(filter(cluster_4_diff, phen_cluster == 1 | phen_cluster == 4), aes(x=G1,y=G2, fill = Profile_clusters_1_4)) +
    geom_point(data = select(cluster_4_diff, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
...
Here, are a subset of 8 substitutions which strongly differentiate between clusters 2 and 3 (100%). This was probably the most difficult comparison to assess in the entire dataset, as although many amino acids are strongly differentiated, there are several caveats. Firstly, there are only 4 PBP-types in the comparison, secondly, there is a low number of isolates in cluster 3 with which to compare isolates of cluster 2, thirdly, isolates of cluster 2 have a large dispersal, with a range of different phenotypes, including a strong outlier. Also, there is only a very small phenotypic distance between the isolates of cluster 2 and 3. Because of these factors, I decided to be conservative and mark these as 'weak' evidence, rather than 'strong', which indicates a potential effect, but where further experimental or comparative evidence is required.
...
cluster_2_cluster_3_difference <- cluster_diff_subs %>% select(Location, AA_max, cluster_2_cluster_3_max) %>%
  filter(cluster_2_cluster_3_max >= .99)
...
cluster_4_diff <- left_join(cluster_4, select(cluster_2_cluster_3_difference, LABID, Profile_clusters_2_3), by = c("LABID")) %>% ungroup()



### Identifying informative isolates --- cluster 2 to 1
A <- ggplot(filter(cluster_diff_subs, cluster_2_cluster_3_max >= 0.98), aes(x = reorder(Location, cluster_2_cluster_3_max, FUN =max), y = cluster_2_cluster_3_max, group = 1)) +
  geom_line(color='black') +
  geom_point(size = 3, shape = 21, color='black', fill = "#377EB8", alpha = 0.8) +
  theme_bw() +
  theme(axis.title=element_text(size=12), 
...
C <- ggplot(filter(cluster_4_diff, phen_cluster == 2 | phen_cluster == 3), aes(x=G1,y=G2, fill = Profile_clusters_2_3)) +
    geom_point(data = select(cluster_4_diff, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
...
ggplot(filter(cluster_2_cluster_3_difference, phen_cluster == 2 | phen_cluster == 3), 
       aes(x=D1,y=D2, fill = Profile_clusters_2_3, colour= Profile_clusters_2_3)) +
   geom_point(data = filter(cluster_2_cluster_3_difference, phen_cluster != 2 | phen_cluster != 1 | phen_cluster != is.na(NA)), 
             colour = "black", fill = "#808080", shape = 21, size = 3, alpha = 1) +
    geom_point(data = cluster_2_cluster_3_difference, 
...
Here there are several substitutions which are strongly differentiated between cluseters 2 and 4. however, cluster 4 only has a single isolate in it, and so these substitutions were marked as weak evidence only.
...
cluster_4_diff <- left_join(cluster_4, select(cluster_2_cluster_4_difference, LABID, Profile_clusters_2_4), by = c("LABID")) %>% ungroup()
...
C <- ggplot(filter(cluster_4_diff, phen_cluster == 2 | phen_cluster == 4), aes(x=G1,y=G2, fill = Profile_clusters_2_4)) +
    geom_point(data = select(cluster_4_diff, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
...
Between clusters 3 and 4, there were again several substitutions which are strongly differentiated. However, cluster 4 only has a single isolate in it, and so these substitutions were marked as weak evidence only.
...
cluster_4_diff <- left_join(cluster_4, select(cluster_3_cluster_4_difference, LABID, Profile_clusters_3_4), by = c("LABID")) %>% ungroup()
...
C <- ggplot(filter(cluster_4_diff, phen_cluster == 3 | phen_cluster == 4), aes(x=G1,y=G2, fill = Profile_clusters_3_4)) +
    geom_point(data = select(cluster_4_diff, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
...
# Summary
In total, 41 unique amino acid changes were weakly associated with the phenotypic cluster differences in this subset of isolates. 
...
write.csv(Potential_substitutions, file = "Sig_AA_subs_Cluster_gen4_S.pneumo.csv", row.names=FALSE)
