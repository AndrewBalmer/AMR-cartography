...
ggplot(filter(AA_matrix_phenotype, gen_cluster == 6), aes(x=G1,y=G2, fill = gen_cluster)) +
...
cluster_6 <- filter(AA_matrix_phenotype, gen_cluster == 6)



## Pull out the AA sequences for the isolates
PBP_positions <- cluster_6 %>% 
  ungroup() %>%
  select(starts_with("PBP"), phen_cluster) %>%
  select(-PBP_type)

    
limits_phen_map_x_cluster <- c(min(cluster_6$D1, na.rm = T), 
                       max(cluster_6$D1, na.rm = T))
limits_phen_map_y_cluster <- c(min(cluster_6$D2, na.rm = T), 
                       max(cluster_6$D2, na.rm = T))

breaks_phen_map_x_cluster <- seq(min(cluster_6$D1, na.rm = T) - (1), 
                         max(cluster_6$D1, na.rm = T) + (1), 1)
breaks_phen_map_y_cluster <- seq(min(cluster_6$D2, na.rm = T) - (1), 
                         max(cluster_6$D2, na.rm = T) + (1), 1)


limits_gen_map_x_cluster <- c(min(cluster_6$G1, na.rm = T) - (gen_slope * 5), 
                      max(cluster_6$G1, na.rm = T) + (gen_slope * 5))
limits_gen_map_y_cluster <- c(min(cluster_6$G2, na.rm = T) - (gen_slope * 5), 
                      max(cluster_6$G2, na.rm = T)+ (gen_slope * 5))

breaks_gen_map_x_cluster <- seq(min(cluster_6$G1, na.rm = T)- (gen_slope * 10), 
                        max(cluster_6$G1, na.rm = T) + (gen_slope * 10), gen_slope * 10)
breaks_gen_map_y_cluster <- seq(min(cluster_6$G2, na.rm = T)- (gen_slope * 10), 
                        max(cluster_6$G2, na.rm = T) + (gen_slope * 10), gen_slope * 10)

ggplot(filter(cluster_6, gen_cluster == 6), aes(x=G1,y=G2, fill = gen_cluster)) +
    geom_point(data = select(all_isolates, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
...
Here, I have plotted the isolates of cluster 6 on the phenotype map. Each colour represents a different genotype. The CDC uses a PBP typing scheme, where the genotype of each of the three PBPs is given a number. The labels represent the particular genotype combination for each of the three proteins.

```{r, echo = F}


ggplot(filter(cluster_6, gen_cluster == 6), aes(x=x_centroid,y=y_centroid, fill = PBP_type)) +
  geom_point(data = select(all_isolates, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
...
ggplot(filter(cluster_6, gen_cluster == 6), aes(x=G1,y=G2, fill = PBP_type)) +
    geom_point(data = select(all_isolates, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
  geom_point(shape = 21,size = 5, alpha = .6) +
  theme_linedraw() +
...
ggplot(filter(AA_matrix_phenotype, gen_cluster == 6), aes(x=D1,y=D2, fill = PBP_type)) +
  geom_point(data = select(AA_matrix_phenotype, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
  geom_point(shape = 21,size = 4, alpha = 1) +
  geom_point(cluster_6, mapping = aes(x_centroid, y_centroid, colour = "#000000"),
...
I then used the hierarchical clustering algorithm on the centroid positions to pick out the 2 regions which have strongly differing phenotypes within this group of low PBP genetic diversity. Here, the centroids and the isolates which belong to that centroid are coloured by their phenotype cluster.

```{r, echo =F}

ggplot(filter(cluster_6), aes(x=D1,y=D2, fill = phen_cluster)) +
  geom_point(data = select(all_isolates, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
    geom_point(shape = 21,size = 4, alpha = 1) +
  geom_point(cluster_6, mapping = aes(x_centroid, y_centroid, colour = phen_cluster),
...
Below is an example of this method. In this case there are no substitutions which clearly segregate between the two clusters. Though there does appear to be a phenotypic difference between the isolates of the two clusters, there are only 5 PBP-types in cluster 1, and each of these only have 1 isolate each. This may mean there is noise in their relative positions, which results in no single substitution coming out as associated with the difference between the two groups. It is therefore not possible to mark any of these substitutions as having evidence in generating a phenotypic effect.
...
cluster_1_cluster_2_difference <- cluster_diff_subs %>% select(Location, AA_max, cluster_1_cluster_2_max) %>%
  filter(cluster_1_cluster_2_max >= .6)
...
cluster_6_diff <- left_join(cluster_6, select(cluster_1_cluster_2_difference, LABID, Profile_clusters_1_2), by = c("LABID")) %>% ungroup()


### Identifying informative isolates --- cluster 2 to 1
A <- ggplot(filter(cluster_diff_subs, cluster_1_cluster_2_max >= 0.4), aes(x = reorder(Location, cluster_1_cluster_2_max, FUN =max), y = cluster_1_cluster_2_max, group = 1)) +
...
C <- ggplot(filter(cluster_6_diff, phen_cluster == 2 | phen_cluster == 1), aes(x=G1,y=G2, fill = Profile_clusters_1_2)) +
    geom_point(data = select(cluster_6_diff, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
...
# Summary
In summary, no PBP locations were strongly associated with the phenotypic cluster differences in this subset of isolates. This is mostly likely because of the low number of isolates for each PBP-type in cluster 1, meaning there is limited confidence to pick out substitutions which may generate a phenotypic change. 
