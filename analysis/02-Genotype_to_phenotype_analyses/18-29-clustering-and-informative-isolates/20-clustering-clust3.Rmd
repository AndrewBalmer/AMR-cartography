...
ggplot(filter(AA_matrix_phenotype, gen_cluster == 3), aes(x=G1,y=G2, fill = gen_cluster)) +
...
cluster_3 <- filter(AA_matrix_phenotype, gen_cluster == 3)



## Pull out the AA sequences for the isolates
PBP_positions <- cluster_3 %>% 
  ungroup() %>%
  select(starts_with("PBP"), phen_cluster) %>%
  select(-PBP_type)

    
limits_phen_map_x_cluster <- c(min(cluster_3$D1, na.rm = T), 
                       max(cluster_3$D1, na.rm = T))
limits_phen_map_y_cluster <- c(min(cluster_3$D2, na.rm = T), 
                       max(cluster_3$D2, na.rm = T))

breaks_phen_map_x_cluster <- seq(min(cluster_3$D1, na.rm = T) - (1), 
                         max(cluster_3$D1, na.rm = T) + (1), 1)
breaks_phen_map_y_cluster <- seq(min(cluster_3$D2, na.rm = T) - (1), 
                         max(cluster_3$D2, na.rm = T) + (1), 1)


limits_gen_map_x_cluster <- c(min(cluster_3$G1, na.rm = T) - (gen_slope * 5), 
                      max(cluster_3$G1, na.rm = T) + (gen_slope * 5))
limits_gen_map_y_cluster <- c(min(cluster_3$G2, na.rm = T) - (gen_slope * 5), 
                      max(cluster_3$G2, na.rm = T)+ (gen_slope * 5))

breaks_gen_map_x_cluster <- seq(min(cluster_3$G1, na.rm = T)- (gen_slope * 10), 
                        max(cluster_3$G1, na.rm = T) + (gen_slope * 10), gen_slope * 10)
breaks_gen_map_y_cluster <- seq(min(cluster_3$G2, na.rm = T)- (gen_slope * 10), 
                        max(cluster_3$G2, na.rm = T) + (gen_slope * 10), gen_slope * 10)

ggplot(filter(cluster_3, gen_cluster == 3), aes(x=G1,y=G2, fill = gen_cluster)) +
    geom_point(data = select(all_isolates, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
...
Here, I have plotted the isolates of cluster 3 on the phenotype map. Each colour represents a different genotype. The CDC uses a PBP typing scheme, where the genotype of each of the three PBPs is given a number. The labels represent the particular genotype combination for each of the three proteins.
...
I then used the hierarchical clustering algorithm on the centroid positions to pick out 3 regions which have differing phenotypes within this group of low PBP genetic diversity. Here, the centroids and the isolates which belong to that centroid are coloured by their phenotype cluster.

As most of the isolates are overlapping and often have lower censored MIC values, it will be difficult to pick out substitutions as there is not a huge phenotypic distance between them. 

```{r, echo =F}

ggplot(filter(cluster_3), aes(x=D1,y=D2, fill = phen_cluster)) +
  geom_point(data = select(all_isolates, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
    geom_point(shape = 21,size = 4, alpha = 1) +
  geom_point(cluster_3, mapping = aes(x_centroid, y_centroid, colour = phen_cluster),
...
In this case there are no substitutions which partially segregate between clusters 1 and 2. 
...
cluster_1_cluster_2_difference <- cluster_diff_subs %>% select(Location, AA_max, cluster_1_cluster_2_max) %>%
  filter(cluster_1_cluster_2_max >= .3)
...
cluster_3_diff <- left_join(cluster_3, select(cluster_1_cluster_2_difference, LABID, Profile_clusters_1_2), by = c("LABID")) %>% ungroup()
...
C <- ggplot(filter(cluster_3_diff, phen_cluster == 2 | phen_cluster == 1), aes(x=G1,y=G2, fill = Profile_clusters_1_2)) +
    geom_point(data = select(cluster_3_diff, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
...
In this case there is 1 substitution which partially segregates between clusters 1 and 3 (>0.80). However, as it does not segregate fully, and so it is only marked as 'weak' evidence.
...
cluster_3_diff <- left_join(cluster_3, select(cluster_1_cluster_3_difference, LABID, Profile_clusters_1_3), by = c("LABID")) %>% ungroup()
...
C <- ggplot(filter(cluster_3_diff, phen_cluster == 1 | phen_cluster == 3), aes(x=G1,y=G2, fill = Profile_clusters_1_3)) +
    geom_point(data = select(cluster_3_diff, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
...
Cluster_substitutions$Genetic_cluster <- "3"
...
In this case there is again 1 substitution which partially segregates between clusters 2 and 3 (>0.80). However, as it does not segregate fully, it is only marked as 'weak' evidence.
...
cluster_2_cluster_3_difference <- cluster_diff_subs %>% select(Location, AA_max, cluster_2_cluster_3_max) %>%
  filter(cluster_2_cluster_3_max >= .8)
...
cluster_3_diff <- left_join(cluster_3, select(cluster_2_cluster_3_difference, LABID, Profile_clusters_2_3), by = c("LABID")) %>% ungroup()
...
C <- ggplot(filter(cluster_3_diff, phen_cluster == 2 | phen_cluster == 3), aes(x=G1,y=G2, fill = Profile_clusters_2_3)) +
    geom_point(data = select(cluster_3_diff, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
...
# Summary
In total, 1 PBP location was weakly associated with the phenotypic cluster differences in this subset of isolates. 
...
write.csv(Potential_substitutions, file = "Sig_AA_subs_Cluster_gen3_S.pneumo.csv", row.names=FALSE)
