...
ggplot(filter(AA_matrix_phenotype, gen_cluster == 8), aes(x=G1,y=G2, fill = gen_cluster)) +
...
cluster_8 <- filter(AA_matrix_phenotype, gen_cluster == 8)



## Pull out the AA sequences for the isolates
PBP_positions <- cluster_8 %>% 
  ungroup() %>%
  select(starts_with("PBP"), phen_cluster) %>%
  select(-PBP_type)

    
limits_phen_map_x_cluster <- c(min(cluster_8$D1, na.rm = T), 
                       max(cluster_8$D1, na.rm = T))
limits_phen_map_y_cluster <- c(min(cluster_8$D2, na.rm = T), 
                       max(cluster_8$D2, na.rm = T))

breaks_phen_map_x_cluster <- seq(min(cluster_8$D1, na.rm = T) - (1), 
                         max(cluster_8$D1, na.rm = T) + (1), 1)
breaks_phen_map_y_cluster <- seq(min(cluster_8$D2, na.rm = T) - (1), 
                         max(cluster_8$D2, na.rm = T) + (1), 1)


limits_gen_map_x_cluster <- c(min(cluster_8$G1, na.rm = T) - (gen_slope * 5), 
                      max(cluster_8$G1, na.rm = T) + (gen_slope * 5))
limits_gen_map_y_cluster <- c(min(cluster_8$G2, na.rm = T) - (gen_slope * 5), 
                      max(cluster_8$G2, na.rm = T)+ (gen_slope * 5))

breaks_gen_map_x_cluster <- seq(min(cluster_8$G1, na.rm = T)- (gen_slope * 10), 
                        max(cluster_8$G1, na.rm = T) + (gen_slope * 10), gen_slope * 10)
breaks_gen_map_y_cluster <- seq(min(cluster_8$G2, na.rm = T)- (gen_slope * 10), 
                        max(cluster_8$G2, na.rm = T) + (gen_slope * 10), gen_slope * 10)

ggplot(filter(cluster_8, gen_cluster == 8), aes(x=G1,y=G2, fill = gen_cluster)) +
    geom_point(data = select(all_isolates, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
...
Here, I have plotted the isolates of cluster 8 on the phenotype map. Each colour represents a different genotype. The CDC uses a PBP typing scheme, where the genotype of each of the three PBPs is given a number. The labels represent the particular genotype combination for each of the three proteins.

```{r, echo = F}


ggplot(filter(cluster_8, gen_cluster == 8), aes(x=x_centroid,y=y_centroid, fill = PBP_type)) +
  geom_point(data = select(all_isolates, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
...
ggplot(filter(cluster_8, gen_cluster == 8), aes(x=G1,y=G2, fill = PBP_type)) +
    geom_point(data = select(all_isolates, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
  geom_point(shape = 21,size = 5, alpha = .6) +
  theme_linedraw() +
...
ggplot(filter(cluster_8), aes(x=D1,y=D2, fill = phen_cluster)) +
  geom_point(data = select(all_isolates, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
    geom_point(shape = 21,size = 4, alpha = 1) +
  geom_point(cluster_8, mapping = aes(x_centroid, y_centroid, colour = phen_cluster),
...
As in the supplementary information, only substitutions marked as having 'Strong' evidence should be considered relevant in generating a phenotypic change, while weak substitutions are those which may have an effect but it is not possible to show with this method.

## Example 1 --- comparing phenotype clusters 1 and 2

In this comparison, there was a difference across 9 amino acid positions between clusters 1 and 2. However, although these differentiated strongly, they were not 100%. Moreover, cluster 2 only had two PBP-types, each of which were only represented by a small number of isolates. Because of these factors, these substitutions were marked as having 'weak' evidence only.
...
cluster_1_cluster_2_difference <- cluster_diff_subs %>% select(Location, AA_max, cluster_1_cluster_2_max) %>%
  filter(cluster_1_cluster_2_max >= .9)
...
cluster_8_diff <- left_join(cluster_8, select(cluster_1_cluster_2_difference, LABID, Profile_clusters_1_2), by = c("LABID")) %>% ungroup()



### Identifying informative isolates --- cluster 2 to 1
A <- ggplot(filter(cluster_diff_subs, cluster_1_cluster_2_max >= 0.65), aes(x = reorder(Location, cluster_1_cluster_2_max, FUN =max), y = cluster_1_cluster_2_max, group = 1)) +
...
C <- ggplot(filter(cluster_8_diff, phen_cluster == 2 | phen_cluster == 1), aes(x=G1,y=G2, fill = Profile_clusters_1_2)) +
    geom_point(data = select(cluster_8_diff, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
...
Cluster_substitutions$Genetic_cluster <- "8"
...
In this comparison, there was a difference between clusters 1 and 3 at 4 amino acid positions. However, these positions did not segregate fully. Consequently, these were only marked as having 'weak' evidence. 
...
cluster_8_diff <- left_join(cluster_8, select(cluster_1_cluster_3_difference, LABID, Profile_clusters_1_3), by = c("LABID")) %>% ungroup()



### Identifying informative isolates --- cluster 1 to 1
A <- ggplot(filter(cluster_diff_subs, cluster_1_cluster_3_max >= 0.58), aes(x = reorder(Location, cluster_1_cluster_3_max, FUN =max), y = cluster_1_cluster_3_max, group = 1)) +
  geom_line(color='black') +
  geom_point(size = 3, shape = 21, color='black', fill = "#377EB8", alpha = 0.8) +
  theme_bw() +
  theme(axis.title=element_text(size=12), 
...
C <- ggplot(filter(cluster_8_diff, phen_cluster == 1 | phen_cluster == 3), aes(x=G1,y=G2, fill = Profile_clusters_1_3)) +
    geom_point(data = select(cluster_8_diff, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
...
In this last comparison, there was a clear difference in amino acid at many positions between clusters 2 and 3. However, in this case, there were only 2 PBP-types in cluster 2, and each of these were only represented by a low number of isolate. Similarly, there were also less than five pbp-types in cluster 3, and each of these was represented by a low number of isolates. Because of this low sample size the distribution of the next most segregated amino acid positions were only around 50%. This effect is typically seen when there are very low numbers of isolates involved in the comparison. Taken together this suggests that while these amino acids may be associated with the change, in this case these changes are likely coming out simply as an artefact of the method. Although they may therefore require further investigation I did not mark them as having evidence for generating a change.
...
cluster_8_diff <- left_join(cluster_8, select(cluster_2_cluster_3_difference, LABID, Profile_clusters_2_3), by = c("LABID")) %>% ungroup()
...
C <- ggplot(filter(cluster_8_diff, phen_cluster == 2 | phen_cluster == 3), aes(x=G1,y=G2, fill = Profile_clusters_2_3)) +
    geom_point(data = select(cluster_8_diff, -gen_cluster), colour = "#969696", fill = "#969696", shape = 16, size = 2.5, alpha = 1)+
...
# Summary
In total, 13 PBP locations were weakly associated with the phenotypic cluster differences in this subset of isolates. 
...
write.csv(Potential_substitutions, file = "Sig_AA_subs_Cluster_gen8_S.pneumo.csv", row.names=FALSE)
