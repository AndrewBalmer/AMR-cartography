...
echo "Submitting $N_PERMUTATIONS permutation arrays, $JOBS chunks each"

for PERM in $(seq 1 "$N_PERMUTATIONS"); do
  bsub -q "$QUEUE" -M "$MEM_MB" -R "select[mem>$MEM_MB] rusage[mem=$MEM_MB]" \
    -J "corr_epi_perm_${PERM}[1-$JOBS]" \
    -oo "$FARM_OUT/logs/perm_${PERM}.%I.%J.out" \
    -eo "$FARM_OUT/logs/perm_${PERM}.%I.%J.err" \
