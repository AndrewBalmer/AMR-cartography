...
MERGED_DIR="${MERGED_DIR:-$FARM_OUT/additive/merged}"
THRESHOLD_DIR="${THRESHOLD_DIR:-$FARM_OUT/thresholds}"
OLD_NULL_FILE="${OLD_NULL_FILE:-$PROJECT_ROOT/AMRC-repo-files/_rerun_snapshots/mvLMM-pre-rerun-20260505-154523/pythonProject1/mvLMM_p_values_normal_pneumo_random_phenotype_FWAS.csv}"
QUEUE="${QUEUE:-normal}"
MEM_MB="${MEM_MB:-8000}"
...
bsub -q "$QUEUE" -M "$MEM_MB" -R "select[mem>$MEM_MB] rusage[mem=$MEM_MB]" \
  -oo "$FARM_OUT/logs/additive_merge.%J.out" \
  -eo "$FARM_OUT/logs/additive_merge.%J.err" \
  "$PYTHON '$SCRIPT_DIR/merge_additive_chunks.py' \
    --chunk-dir '$CHUNK_DIR' \
    --out-dir '$MERGED_DIR' \
    --threshold-dir '$THRESHOLD_DIR' \
    --galwey-meff '$ADD_MEFF' \
    --old-null-file '$OLD_NULL_FILE'"
