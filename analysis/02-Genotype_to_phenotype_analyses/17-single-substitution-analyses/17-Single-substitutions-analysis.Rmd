...
load(amrc_data_path("tablemic_pneumo_3628_meta_gen_distance_matrix.RData"))
...
nat_exp_rankings_summary <- nat_exp_rankings %>%
 group_by(Relative_Comparison,
          ...
          AA_1,
          AA_2) %>%
    # Historical column name retained for compatibility with script 32. The
    # value calculated here is the mean pairwise distance, not the median.
...
individual_subs$Label <- paste(individual_subs$PBP_position, " - Mean:",  round(as.numeric(individual_subs$median_phenotypic_distance), 3),sep="")
...
ggplot(aes(x = Substitution_of_interest, y = median_phenotypic_distance)) + 
  ...
  ylab("Mean pairwise phenotypic distance (log2 MIC units)") +
