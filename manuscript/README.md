...
manuscript's numbering (S14 = single-substitution comparisons, S15 =
single-substitution maps, S18 = epistasis, S19 = effect-size axis
categorisation, S20 = prior-work overlap):

- `Table3_and_SupplementaryTableS14_top20.csv` — top-20 substitutions shown in
  main-text Table 3 and Supplementary **Table** S14.
- `FigureS14_single_substitution_comparisons.{csv,pdf,png}` — the 33
  single-substitution comparisons with a mean pairwise phenotypic distance
  >0.5 log2 MIC units, plotted per PBP. Generated by
  `analysis/02-Genotype_to_phenotype_analyses/17-single-substitution-analyses/plot-S14-single-substitution-comparisons.R`.
- `FigureS15_single_substitution_maps.{pdf,png}` plus its three source tables
  `FigureS15_comparison_summary.csv` (the 15 comparisons with a mean distance
  ≥1 log2 MIC unit, one per panel), `FigureS15_isolate_points.csv` (plotted
  isolate coordinates) and `FigureS15_pairwise_segments.csv` (the pairwise
  connecting lines). Generated by
  `analysis/02-Genotype_to_phenotype_analyses/17-single-substitution-analyses/plot-S15-single-substitution-maps.R`.
...
Note that S14 exists as both a **Table** and a **Figure** in the supplement;
`Table3_and_SupplementaryTableS14_top20.csv` is the table, and the
`FigureS14_*` files are the figure.
